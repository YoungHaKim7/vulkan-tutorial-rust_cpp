...
int main(int argc, char *argv[]) {
    ...
    for (uint32_t i = 0; i < numbers_length; i++) {
        ...
        wgpuComputePassEncoderSetImmediates(compute_pass_encoder, 0, &immediate,
                                            sizeof(uint32_t));
    ...
    wgpuBufferMapAsync(
        staging_buffer, WGPUMapMode_Read, 0, numbers_size,
        (const WGPUBufferMapCallbackInfo){.callback = handle_buffer_map});
