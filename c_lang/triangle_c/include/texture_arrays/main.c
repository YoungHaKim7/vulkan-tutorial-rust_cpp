...
#error "Unsupported GLFW native platform"
    ...
    wgpuSurfaceGetCapabilities(demo.surface, demo.adapter,
    ...
    required_device_native_limits.maxBindingArraySamplerElementsPerShaderStage =
        2;

    WGPULimits required_device_limits = WGPU_LIMITS_INIT;
    required_device_limits.nextInChain =
        (const WGPUChainedStruct *)&required_device_native_limits;
...
#define COLOR_TEXTURE_DESCRIPTOR_COMMON_FIELDS                                 \
  ...
  .usage = WGPUTextureUsage_TextureBinding | WGPUTextureUsage_CopyDst
    ...
    WGPUTextureView blue_texture_view =
        wgpuTextureCreateView(blue_texture, NULL);
...
#define COLOR_IMAGE_COPY_TEXTURE_COMMON_FIELDS                                 \
  ...
  .aspect = WGPUTextureAspect_All
    ...
    const WGPUTexelCopyBufferLayout texture_data_layout_common =
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        if (use_uniform_workaround) {
            ...
            wgpuRenderPassEncoderSetBindGroup(render_pass_encoder, 1,
                                              uniforms_bind_group, 1,
                                              (const uint32_t[]){0});
            wgpuRenderPassEncoderDrawIndexed(render_pass_encoder, 6, 1, 0, 0,
                                             0);
            wgpuRenderPassEncoderSetBindGroup(render_pass_encoder, 1,
                                              uniforms_bind_group, 1,
                                              (const uint32_t[]){256});
            wgpuRenderPassEncoderDrawIndexed(render_pass_encoder, 6, 1, 6, 0,
                                             0);
        } else {
            wgpuRenderPassEncoderSetBindGroup(render_pass_encoder, 0,
                                              bind_group, 0, NULL);
            wgpuRenderPassEncoderSetBindGroup(render_pass_encoder, 1,
                                              uniforms_bind_group, 1,
                                              (const uint32_t[]){0});
            wgpuRenderPassEncoderDrawIndexed(render_pass_encoder, 12, 1, 0, 0,
                                             0);
        }
