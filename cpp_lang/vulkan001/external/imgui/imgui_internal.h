...
enum ImGuiNavDirSourceFlags_ {
  ...
  ImGuiNavDirSourceFlags_RawKeyboard =
      1 << 0, // Raw keyboard (not pulled from nav), faciliate use of some
...
enum ImGuiDockNodeFlagsPrivate_ {
  ...
  ImGuiDockNodeFlags_DockSpace =
      1 << 10, // Local, Saved  // A dockspace is a node that occupy space
               // within an existing user window. Otherwise the node is floating
               // and create its own window.
  ImGuiDockNodeFlags_CentralNode =
      1 << 11, // Local, Saved  // The central node has 2 main properties: stay
               // visible when empty, only use "remaining" spaces from its
               // neighbor.
  ...
  ImGuiDockNodeFlags_HiddenTabBar =
      1 << 13, // Local, Saved  // Tab bar is hidden, with a triangle in the
               // corner to show it again (NB: actual tab-bar instance may be
               // destroyed as this is only used for single-window tab bar)
  ImGuiDockNodeFlags_NoWindowMenuButton =
      1 << 14, // Local, Saved  // Disable window/docking menu (that one that
               // appears instead of the collapse button)
  ImGuiDockNodeFlags_NoCloseButton = 1 << 15, // Local, Saved  //
  ImGuiDockNodeFlags_NoDocking =
      1 << 16, // Local, Saved  // Disable any form of docking in this dockspace
               // or individual node. (On a whole dockspace, this pretty much
               // defeat the purpose of using a dockspace at all). Note: when
               // turned on, existing docked nodes will be preserved.
  ImGuiDockNodeFlags_NoDockingSplitMe =
      1 << 17, // [EXPERIMENTAL] Prevent another window/node from splitting this
               // node.
  ImGuiDockNodeFlags_NoDockingSplitOther =
      1 << 18, // [EXPERIMENTAL] Prevent this node from splitting another
               // window/node.
  ImGuiDockNodeFlags_NoDockingOverMe =
      1 << 19, // [EXPERIMENTAL] Prevent another window/node to be docked over
               // this node.
  ImGuiDockNodeFlags_NoDockingOverOther =
      1 << 20, // [EXPERIMENTAL] Prevent this node to be docked over another
               // window or non-empty node.
  ImGuiDockNodeFlags_NoDockingOverEmpty =
      1 << 21, // [EXPERIMENTAL] Prevent this node to be docked over an empty
  ...
  ImGuiDockNodeFlags_LocalFlagsTransferMask_ =
      ImGuiDockNodeFlags_LocalFlagsMask_ &
      ~ImGuiDockNodeFlags_DockSpace, // When splitting those flags are moved to
...
struct ImGuiViewportP : public ImGuiViewport {
  ...
  int LastFrameActive; // Last frame number this viewport was activated by a
                       // window
  int LastFrontMostStampCount; // Last stamp number from when a window hosted by
                               // this viewport was made front-most (by
...
struct ImGuiStackLevelInfo {
  ...
  char Desc[58]; // Arbitrarily sized buffer to hold a result (FIXME: could
                 // replace Results[] with a chunk stream?)
...
struct ImGuiContext {
  ...
  int ViewportFrontMostStampCount; // Every time the front-most window changes,
...
struct IMGUI_API ImGuiWindow {
  ...
  ImGuiViewportP *Viewport; // Always set in Begin(), only inactive windows may
...
enum ImGuiTabItemFlagsPrivate_ {
  ...
  ImGuiTabItemFlags_Preview =
      1 << 23 // [Docking] Display tab shape for docking preview (height is
              // adjusted slightly to compensate for the yet missing tab bar)
};
