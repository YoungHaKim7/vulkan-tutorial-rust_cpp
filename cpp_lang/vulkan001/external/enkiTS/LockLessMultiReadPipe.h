...
namespace enki {
// LockLessMultiReadPipe - Single writer, multiple reader thread safe pipe using
// (semi) lockless programming Readers can only read from the back of the pipe
// The single writer can write to the front of the pipe, and read from both ends
// (a writer can be a reader) for many of the principles used here, see
...
public:
  ...
  // This is thread safe for the single writer, but should not be called by
  // readers
  ...
  bool IsPipeEmpty() const {
    return 0 == m_WriteIndex.load(std::memory_order_relaxed) -
...
inline LockLessMultiReadPipe<cSizeLog2, T>::LockLessMultiReadPipe()
    : m_WriteIndex(0), m_ReadCount(0), m_ReadIndex(0) {
...
inline bool LockLessMultiReadPipe<cSizeLog2, T>::ReaderTryReadBack(T *pOut) {
  ...
  while (true) {
    ...
    bool bSuccess = m_Flags[actualReadIndex].compare_exchange_strong(
        previous, FLAG_INVALID, std::memory_order_acq_rel,
  ...
  // we update the read index using an atomic add, as we've only read one piece
  // of data. this ensure consistency of the read index, and the above loop
  // ensures readers only read from unread data
...
inline bool LockLessMultiReadPipe<cSizeLog2, T>::WriterTryReadFront(T *pOut) {
  ...
  // Multiple potential readers mean we should check if the data is valid,
  // using an atomic compare exchange - which acts as a form of lock (so not
  // quite lockless really).
...
template <uint8_t cSizeLog2, typename T>
inline bool
LockLessMultiReadPipe<cSizeLog2, T>::WriterTryWriteFront(const T &in) {
  ...
  if (m_Flags[actualWriteIndex].load(std::memory_order_acquire) !=
      FLAG_CAN_WRITE) {
  ...
  T *ReaderReadBack() {
    ...
    if (pTailPlus1) {
      ...
      if (pTailPlus2) {
        ...
        tail.pNext = NULL;
        T *pCompare = pTailPlus1; // we need preserve pTailPlus1 as compare will
                                  // alter it on failure
        ...
        if (!pHead.compare_exchange_strong(pCompare, &tail)) {
          ...
          // pTailPlus1 is no longer the head, so pTailPlus1->pNext should be
          // non NULL
