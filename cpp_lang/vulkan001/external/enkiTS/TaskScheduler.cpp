...
static const int32_t gc_TaskAlmostCompleteCount =
    1; // GetIsComplete() will return false, but execution is done and about to
       // complete
...
// each software thread gets it's own copy of gtl_threadNum, so this is safe to
// use as a static variable
...
// we derive class TaskPipe rather than typedef to get forward declaration
// working easily
class TaskPipe
    : public LockLessMultiReadPipe<gc_PipeSizeLog2, enki::SubTaskSet> {};

enum ThreadState : int32_t {
  ENKI_THREAD_STATE_NONE,         // shouldn't get this value
  ENKI_THREAD_STATE_NOT_LAUNCHED, // for debug purposes - indicates enki task
                                  // thread not yet launched
  ENKI_THREAD_STATE_RUNNING,
  ENKI_THREAD_STATE_PRIMARY_REGISTERED, // primary thread is the one enkiTS was
                                        // initialized on
...
struct alignas(enki::gc_CacheLineSize) ThreadDataStore {
  ...
  char prevent_false_Share[enki::gc_CacheLineSize -
                           sizeof(std::atomic<ThreadState>) -
                           sizeof(semaphoreid_t *)];
};
constexpr size_t SIZEOFTHREADDATASTORE =
    sizeof(ThreadDataStore); // for easier inspection
static_assert(SIZEOFTHREADDATASTORE == enki::gc_CacheLineSize,
              "ThreadDataStore may exhibit false sharing");
...
ENKITS_API void *enki::DefaultAllocFunc(size_t align_, size_t size_,
                                        void *userData_, const char *file_,
                                        int line_) {
  (void)userData_;
  (void)file_;
  (void)line_;
...
ENKITS_API void enki::DefaultFreeFunc(void *ptr_, size_t size_, void *userData_,
...
bool TaskScheduler::RegisterExternalTaskThread() {
  ...
  while (!bRegistered && m_NumExternalTaskThreadsRegistered <
                             (int32_t)m_Config.numExternalTaskThreads) {
    for (uint32_t thread = GetNumFirstExternalTaskThread();
         thread <
         GetNumFirstExternalTaskThread() + m_Config.numExternalTaskThreads;
         ++thread) {
...
bool TaskScheduler::RegisterExternalTaskThread(uint32_t threadNumToRegister_) {
  ...
  ENKI_ASSERT(threadNumToRegister_ < (GetNumFirstExternalTaskThread() +
                                      m_Config.numExternalTaskThreads));
  ThreadState threadStateExpected = ENKI_THREAD_STATE_EXTERNAL_UNREGISTERED;
  if (m_pThreadDataStore[threadNumToRegister_]
          .threadState.compare_exchange_strong(
              threadStateExpected, ENKI_THREAD_STATE_EXTERNAL_REGISTERED)) {
    ++m_NumExternalTaskThreadsRegistered;
...
void TaskScheduler::DeRegisterExternalTaskThread() {
  ...
  ThreadState threadState = m_pThreadDataStore[gtl_threadNum].threadState.load(
      std::memory_order_acquire);
  ENKI_ASSERT(threadState == ENKI_THREAD_STATE_EXTERNAL_REGISTERED);
  if (threadState == ENKI_THREAD_STATE_EXTERNAL_REGISTERED) {
    --m_NumExternalTaskThreadsRegistered;
    m_pThreadDataStore[gtl_threadNum].threadState.store(
        ENKI_THREAD_STATE_EXTERNAL_UNREGISTERED, std::memory_order_release);
    gtl_threadNum = 0;
  }
}
...
void TaskScheduler::TaskingThreadFunction(const ThreadArgs &args_) {
  ...
  pTS->m_pThreadDataStore[threadNum].threadState.store(
      ENKI_THREAD_STATE_RUNNING, std::memory_order_release);
  ...
  pTS->m_pThreadDataStore[threadNum].threadState.store(
      ENKI_THREAD_STATE_STOPPED, std::memory_order_release);
...
void TaskScheduler::StartThreads() {
  ...
  m_NumThreads =
      m_Config.numTaskThreadsToCreate + m_Config.numExternalTaskThreads + 1;

  for (int priority = 0; priority < TASK_PRIORITY_NUM; ++priority) {
    m_pPipesPerThread[priority] =
        NewArray<TaskPipe>(m_NumThreads, ENKI_FILE_AND_LINE);
    m_pPinnedTaskListPerThread[priority] =
        NewArray<PinnedTaskList>(m_NumThreads, ENKI_FILE_AND_LINE);
  }

  m_pNewTaskSemaphore = SemaphoreNew();
  m_pTaskCompleteSemaphore = SemaphoreNew();

  // we create one less thread than m_NumThreads as the main thread counts as
  // one
  m_pThreadDataStore =
      NewArray<ThreadDataStore>(m_NumThreads, ENKI_FILE_AND_LINE);
  ...
  for (uint32_t thread = GetNumFirstExternalTaskThread();
       thread <
       m_Config.numExternalTaskThreads + GetNumFirstExternalTaskThread();
       ++thread) {
    m_pThreadDataStore[thread].threadState =
        ENKI_THREAD_STATE_EXTERNAL_UNREGISTERED;
  }
  for (uint32_t thread =
           m_Config.numExternalTaskThreads + GetNumFirstExternalTaskThread();
       thread < m_NumThreads; ++thread) {
  ...
  for (uint32_t thread =
       ...
       thread < m_NumThreads; ++thread) {
    m_pThreads[thread] =
        std::thread(TaskingThreadFunction, ThreadArgs{thread, this});
  ...
  // ensure we have sufficient tasks to equally fill either all threads
  // including main or just the threads we've launched, this is outside the
  // firstinit as we want to be able to runtime change it
  if (1 == m_NumThreads) {
    m_NumPartitions = 1;
    ...
    // We only need to partition for a maximum of the available processor
    // parallelism.
    uint32_t numThreadsToPartitionFor =
        std::min(m_NumThreads, GetNumHardwareThreads());
    m_NumPartitions = numThreadsToPartitionFor * (numThreadsToPartitionFor - 1);
    // ensure m_NumPartitions, m_NumInitialPartitions non zero, can happen if
    // m_NumThreads > 1 && GetNumHardwareThreads() == 1
    m_NumPartitions = std::max(m_NumPartitions, (uint32_t)1);
    m_NumInitialPartitions =
        std::max(numThreadsToPartitionFor - 1, (uint32_t)1);
  ...
  if (m_bHaveThreads) {
    ...
    while (bWait_ && m_NumInternalTaskThreadsRunning) {
      ...
      for (uint32_t threadId = 0; threadId < m_NumThreads; ++threadId) {
        // send wait for new pinned tasks signal to ensure any waiting are
        // awoken
        SemaphoreSignal(
            *m_pThreadDataStore[threadId].pWaitNewPinnedTaskSemaphore, 1);
      }
    }

    // detach threads starting with thread GetNumFirstExternalTaskThread() (as 0
    // is initialization thread).
    for (uint32_t thread =
             m_Config.numExternalTaskThreads + GetNumFirstExternalTaskThread();
         thread < m_NumThreads; ++thread) {
    ...
    for (uint32_t threadNum = 0; threadNum < m_NumThreads; ++threadNum) {
      SemaphoreDelete(
          m_pThreadDataStore[threadNum].pWaitNewPinnedTaskSemaphore);
    }
...
bool TaskScheduler::TryRunTask(uint32_t threadNum_,
                               uint32_t &hintPipeToCheck_io_) {
  for (int priority = 0; priority < TASK_PRIORITY_NUM; ++priority) {
...
bool TaskScheduler::TryRunTask(uint32_t threadNum_, uint32_t priority_,
                               uint32_t &hintPipeToCheck_io_) {
  ...
  SubTaskSet subTask;
  bool bHaveTask =
      m_pPipesPerThread[priority_][threadNum_].WriterTryReadFront(&subTask);
  ...
  while (!bHaveTask && checkCount < m_NumThreads) {
    ...
    if (threadToCheck != threadNum_) {
      bHaveTask = m_pPipesPerThread[priority_][threadToCheck].ReaderTryReadBack(
          &subTask);
    }
    ++checkCount;
  }
  ...
  if (bHaveTask) {
    // update hint, will preserve value unless actually got task from another
    // thread.
    ...
    if (subTask.pTask->m_RangeToRun < partitionSize) {
      ...
      int prevCount = taskToRun.pTask->m_RunningCount.fetch_sub(
          1, std::memory_order_release);
      if (gc_TaskStartCount == prevCount) {
        TaskComplete(taskToRun.pTask, true, threadNum_);
      }
    } else {
      // the task has already been divided up by AddTaskSetToPipe, so just run
      // it
...
void TaskScheduler::TaskComplete(ICompletable *pTask_, bool bWakeThreads_,
                                 uint32_t threadNum_) {
  // It must be impossible for a thread to enter the sleeping wait prior to the
  // load of m_WaitingForTaskCount in this function, so we introduce an
  // gc_TaskAlmostCompleteCount to prevent this.
  ENKI_ASSERT(gc_TaskAlmostCompleteCount ==
              pTask_->m_RunningCount.load(std::memory_order_acquire));
  bool bCallWakeThreads = bWakeThreads_ && pTask_->m_WaitingForTaskCount.load(
                                               std::memory_order_acquire);
  ...
  while (pDependent) {
    int prevDeps =
        pDependent->pTaskToRunOnCompletion->m_DependenciesCompletedCount
            .fetch_add(1, std::memory_order_release);
    ENKI_ASSERT(prevDeps <
                pDependent->pTaskToRunOnCompletion->m_DependenciesCount);
    if (pDependent->pTaskToRunOnCompletion->m_DependenciesCount ==
        (prevDeps + 1)) {
      // get temp copy of pDependent so OnDependenciesComplete can delete task
      // if needed.
      Dependency *pDependentCurr = pDependent;
      pDependent = pDependent->pNext;
      // reset dependencies
      pDependentCurr->pTaskToRunOnCompletion->m_DependenciesCompletedCount
          .store(0, std::memory_order_release);
      pDependentCurr->pTaskToRunOnCompletion->OnDependenciesComplete(
          this, threadNum_);
    } else {
      pDependent = pDependent->pNext;
    }
...
void TaskScheduler::WaitForNewTasks(uint32_t threadNum_) {
  ...
  ThreadState prevThreadState = m_pThreadDataStore[threadNum_].threadState.load(
      std::memory_order_relaxed);
  m_pThreadDataStore[threadNum_].threadState.store(
      ENKI_THREAD_STATE_WAIT_NEW_TASKS, std::memory_order_seq_cst);

  if (HaveTasks(threadNum_)) {
    m_NumThreadsWaitingForNewTasks.fetch_sub(1, std::memory_order_release);
  } else {
    SafeCallback(m_Config.profilerCallbacks.waitForNewTaskSuspendStart,
                 threadNum_);
    SemaphoreWait(*m_pNewTaskSemaphore);
    SafeCallback(m_Config.profilerCallbacks.waitForNewTaskSuspendStop,
                 threadNum_);
  }

  m_pThreadDataStore[threadNum_].threadState.store(prevThreadState,
...
void TaskScheduler::WaitForTaskCompletion(const ICompletable *pCompletable_,
  ...
  ThreadState prevThreadState = m_pThreadDataStore[threadNum_].threadState.load(
      std::memory_order_relaxed);
  m_pThreadDataStore[threadNum_].threadState.store(
      ENKI_THREAD_STATE_WAIT_TASK_COMPLETION, std::memory_order_seq_cst);

  // do not wait on semaphore if task in gc_TaskAlmostCompleteCount state.
  if (gc_TaskAlmostCompleteCount >=
          pCompletable_->m_RunningCount.load(std::memory_order_acquire) ||
      HaveTasks(threadNum_)) {
    m_NumThreadsWaitingForTaskCompletion.fetch_sub(1,
                                                   std::memory_order_release);
  } else {
    SafeCallback(m_Config.profilerCallbacks.waitForTaskCompleteSuspendStart,
                 threadNum_);
    ...
    SafeCallback(m_Config.profilerCallbacks.waitForTaskCompleteSuspendStop,
                 threadNum_);
  }
...
void TaskScheduler::WakeThreadsForNewTasks() {
  int32_t waiting =
      m_NumThreadsWaitingForNewTasks.load(std::memory_order_relaxed);
  while (waiting > 0 && !m_NumThreadsWaitingForNewTasks.compare_exchange_weak(
                            waiting, 0, std::memory_order_release,
                            std::memory_order_relaxed)) {
  }
...
void TaskScheduler::WakeThreadsForTaskCompletion() {
  ...
  int32_t waiting =
      m_NumThreadsWaitingForTaskCompletion.load(std::memory_order_relaxed);
  while (
      waiting > 0 &&
      !m_NumThreadsWaitingForTaskCompletion.compare_exchange_weak(
...
bool TaskScheduler::WakeSuspendedThreadsWithPinnedTasks() {
  ...
  for (uint32_t t = 1; t < m_NumThreads; ++t) {
    // distribute thread checks more evenly by starting at our thread number
    // rather than 0.
    uint32_t thread = (threadNum + t) % m_NumThreads;

    ThreadState state =
        m_pThreadDataStore[thread].threadState.load(std::memory_order_acquire);

    ENKI_ASSERT(state != ENKI_THREAD_STATE_NONE);

    if (state == ENKI_THREAD_STATE_WAIT_NEW_TASKS ||
        state == ENKI_THREAD_STATE_WAIT_TASK_COMPLETION) {
...
void TaskScheduler::SplitAndAddTask(uint32_t threadNum_, SubTaskSet subTask_,
                                    uint32_t rangeToSplit_) {
  int32_t numAdded = 0;
  int32_t numRun = 0;
  // ensure that an artificial completion is not registered whilst adding tasks
  ...
  while (subTask_.partition.start != subTask_.partition.end) {
    ...
    ++numAdded;
    subTask_.pTask->m_RunningCount.fetch_add(1, std::memory_order_acquire);
    if (!m_pPipesPerThread[subTask_.pTask->m_Priority][threadNum_]
             .WriterTryWriteFront(taskToAdd)) {
      if (numAdded > 1) {
        WakeThreadsForNewTasks();
      }
      numAdded = 0;
      // alter range to run the appropriate fraction
      if (taskToAdd.pTask->m_RangeToRun <
          taskToAdd.partition.end - taskToAdd.partition.start) {
        taskToAdd.partition.end =
            taskToAdd.partition.start + taskToAdd.pTask->m_RangeToRun;
  ...
  int prevCount = subTask_.pTask->m_RunningCount.fetch_sub(
      numRun + 1, std::memory_order_release);
  if (numRun + gc_TaskStartCount == prevCount) {
    TaskComplete(subTask_.pTask, false, threadNum_);
  }

  // WakeThreadsForNewTasks also calls WakeThreadsForTaskCompletion() so do not
  // need to do so above
  WakeThreadsForNewTasks();
}
...
void TaskScheduler::AddTaskSetToPipeInt(ITaskSet *pTaskSet_,
                                        uint32_t threadNum_) {
  ENKI_ASSERT(pTaskSet_->m_RunningCount == gc_TaskStartCount);
  ThreadState prevThreadState = m_pThreadDataStore[threadNum_].threadState.load(
      std::memory_order_relaxed);
  m_pThreadDataStore[threadNum_].threadState.store(ENKI_THREAD_STATE_RUNNING,
                                                   std::memory_order_relaxed);
  std::atomic_thread_fence(std::memory_order_acquire);
  ...
  int prevCount =
      pTaskSet_->m_RunningCount.fetch_sub(1, std::memory_order_release);
  if (gc_TaskStartCount == prevCount) {
...
void TaskScheduler::AddPinnedTaskInt(IPinnedTask *pTask_) {
  ...
  m_pPinnedTaskListPerThread[pTask_->m_Priority][pTask_->threadNum]
      .WriterWriteFront(pTask_);

  ThreadState statePinnedTaskThread =
      m_pThreadDataStore[pTask_->threadNum].threadState.load(
          std::memory_order_acquire);
  if (statePinnedTaskThread == ENKI_THREAD_STATE_WAIT_NEW_PINNED_TASKS) {
    SemaphoreSignal(
        *m_pThreadDataStore[pTask_->threadNum].pWaitNewPinnedTaskSemaphore, 1);
  } else {
    WakeThreadsForNewTasks();
  }
}
...
void TaskScheduler::InitDependencies(ICompletable *pCompletable_) {
  // go through any dependencies and set thier running count so they show as not
  // complete and increment depedency count
  ...
  while (pDependent) {
    ...
    pDependent->pTaskToRunOnCompletion->m_RunningCount.store(
        gc_TaskStartCount, std::memory_order_relaxed);
    pDependent = pDependent->pNext;
  }
}
...
void TaskScheduler::RunPinnedTasks() {
  uint32_t threadNum = gtl_threadNum;
  ThreadState prevThreadState =
      m_pThreadDataStore[threadNum].threadState.load(std::memory_order_relaxed);
  m_pThreadDataStore[threadNum].threadState.store(ENKI_THREAD_STATE_RUNNING,
                                                  std::memory_order_relaxed);
  std::atomic_thread_fence(std::memory_order_acquire);
  for (int priority = 0; priority < TASK_PRIORITY_NUM; ++priority) {
    RunPinnedTasks(threadNum, priority);
  }
  m_pThreadDataStore[threadNum].threadState.store(prevThreadState,
                                                  std::memory_order_release);
}
...
void TaskScheduler::RunPinnedTasks(uint32_t threadNum_, uint32_t priority_) {
  ...
  do {
    pPinnedTaskSet =
        m_pPinnedTaskListPerThread[priority_][threadNum_].ReaderReadBack();
    if (pPinnedTaskSet) {
...
void TaskScheduler::WaitforTask(const ICompletable *pCompletable_,
                                enki::TaskPriority priorityOfLowestToRun_) {
  uint32_t threadNum = gtl_threadNum;
  uint32_t hintPipeToCheck_io = threadNum + 1; // does not need to be clamped.

  // waiting for a task is equivalent to 'running' for thread state purpose as
  // we may run tasks whilst waiting
  ThreadState prevThreadState =
      m_pThreadDataStore[threadNum].threadState.load(std::memory_order_relaxed);
  m_pThreadDataStore[threadNum].threadState.store(ENKI_THREAD_STATE_RUNNING,
                                                  std::memory_order_relaxed);
  ...
  if (pCompletable_ && !pCompletable_->GetIsComplete()) {
    SafeCallback(m_Config.profilerCallbacks.waitForTaskCompleteStart,
                 threadNum);
    // We need to ensure that the task we're waiting on can complete even if
    // we're the only thread, so we clamp the priorityOfLowestToRun_ to no
    // smaller than the task we're waiting for
    priorityOfLowestToRun_ =
        std::max(priorityOfLowestToRun_, pCompletable_->m_Priority);
    uint32_t spinCount = 0;
...
void TaskScheduler::WaitforAll() {
  ...
  uint32_t hintPipeToCheck_io =
      ourThreadNum + 1;             // does not need to be clamped.
  ...
  while (bHaveTasks || otherThreadsRunning) {
    ...
    if (spinCount > gc_SpinCount) {
      ...
      do {
        ...
        // We can only add a pinned task to wait on if we find an enki Task
        // Thread which isn't this thread. Otherwise we have to busy wait.
        if (dummyWaitTask.threadNum != ourThreadNum &&
            dummyWaitTask.threadNum > m_Config.numExternalTaskThreads) {
          ThreadState state =
              m_pThreadDataStore[dummyWaitTask.threadNum].threadState.load(
                  std::memory_order_acquire);
          if (state == ENKI_THREAD_STATE_RUNNING ||
              state == ENKI_THREAD_STATE_WAIT_TASK_COMPLETION) {
    ...
    for (uint32_t thread = 0; thread < m_NumThreads && !otherThreadsRunning;
         ++thread) {
      // ignore our thread
      if (thread != ourThreadNum) {
        switch (m_pThreadDataStore[thread].threadState.load(
            std::memory_order_acquire)) {
        ...
        case ENKI_THREAD_STATE_WAIT_NEW_PINNED_TASKS:
          otherThreadsRunning = true;
          SemaphoreSignal(
              *m_pThreadDataStore[thread].pWaitNewPinnedTaskSemaphore, 1);
          break;
...
void TaskScheduler::WaitForNewPinnedTasks() {
  ...
  ThreadState prevThreadState =
      m_pThreadDataStore[threadNum].threadState.load(std::memory_order_relaxed);
  m_pThreadDataStore[threadNum].threadState.store(
      ENKI_THREAD_STATE_WAIT_NEW_PINNED_TASKS, std::memory_order_seq_cst);
...
T *TaskScheduler::NewArray(size_t num_, const char *file_, int line_) {
  T *pRet = (T *)m_Config.customAllocator.alloc(
      alignof(T), num_ * sizeof(T), m_Config.customAllocator.userData, file_,
      line_);
...
template <typename T>
void TaskScheduler::DeleteArray(T *p_, size_t num_, const char *file_,
                                int line_) {
  if (!std::is_pod<T>::value) {
    size_t i = num_;
    while (i) {
  ...
  m_Config.customAllocator.free(
      p_, sizeof(T) * num_, m_Config.customAllocator.userData, file_, line_);
}
...
template <typename T> T *TaskScheduler::Alloc(const char *file_, int line_) {
  T *pRet = (T *)m_Config.customAllocator.alloc(
      alignof(T), sizeof(T), m_Config.customAllocator.userData, file_, line_);
  return pRet;
}
...
void TaskScheduler::Free(T *p_, const char *file_, int line_) {
  m_Config.customAllocator.free(
      p_, sizeof(T), m_Config.customAllocator.userData, file_, line_);
}

TaskScheduler::TaskScheduler()
    : m_pPipesPerThread(), m_pPinnedTaskListPerThread(), m_NumThreads(0),
      m_pThreadDataStore(NULL), m_pThreads(NULL), m_bRunning(0),
      m_NumInternalTaskThreadsRunning(0), m_NumThreadsWaitingForNewTasks(0),
      m_NumThreadsWaitingForTaskCompletion(0), m_NumPartitions(0),
      m_pNewTaskSemaphore(NULL), m_pTaskCompleteSemaphore(NULL),
      m_NumInitialPartitions(0), m_bHaveThreads(false),
      m_NumExternalTaskThreadsRegistered(0) {}
...
#ifdef _XBOX_ONE
  semaphoreid.sem =
      CreateSemaphoreExW(NULL, 0, MAXLONG, NULL, 0, SEMAPHORE_ALL_ACCESS);
#else
...
// Dispatch semaphores appear to be the way other developers use OSX Semaphores,
// e.g. Boost However the API could change OSX below 10.6 does not support
// dispatch, but I do not have an earlier OSX version to test alternatives
...
Dependency::Dependency(const ICompletable *pDependencyTask_,
                       ICompletable *pTaskToRunOnCompletion_)
    : pTaskToRunOnCompletion(pTaskToRunOnCompletion_),
      pDependencyTask(pDependencyTask_), pNext(pDependencyTask->m_pDependents) {
...
void Dependency::SetDependency(const ICompletable *pDependencyTask_,
                               ICompletable *pTaskToRunOnCompletion_) {
  ClearDependency();
