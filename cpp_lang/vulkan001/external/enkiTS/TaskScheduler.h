...
// Define ENKI_CUSTOM_ALLOC_FILE_AND_LINE (at project level) to get file and
// line report in custom allocators, this is default in Debug - to turn off
// define ENKI_CUSTOM_ALLOC_NO_FILE_AND_LINE
...
public:
  ...
  void SetDependency(Dependency &dependency_,
                     const ICompletable *pDependencyTask_);
  template <typename D, typename T, int SIZE>
  void SetDependenciesArr(D &dependencyArray_, const T (&taskArray_)[SIZE]);
  template <typename D, typename T>
  void SetDependenciesArr(D &dependencyArray_,
                          std::initializer_list<T *> taskpList_);
  template <typename D, typename T, int SIZE>
  void SetDependenciesArr(D (&dependencyArray_)[SIZE],
                          const T (&taskArray_)[SIZE]);
  template <typename D, typename T, int SIZE>
  void SetDependenciesArr(D (&dependencyArray_)[SIZE],
                          std::initializer_list<T *> taskpList_);
  template <typename D, typename T, int SIZE>
  ...
  template <typename D, typename T>
  void SetDependenciesVec(D &dependencyVec_,
                          std::initializer_list<T *> taskpList_);
...
protected:
  ...
  virtual void OnDependenciesComplete(TaskScheduler *pTaskScheduler_,
                                      uint32_t threadNum_);
...
public:
  ...
  // Execute range should be overloaded to process tasks. It will be called with
  // a range_ where range.start >= 0; range.start < range.end; and range.end <
  // m_SetSize; The range values should be mapped so that linearly processing
  // them in order is cache friendly i.e. neighbouring values should be close
  // together. threadnum should not be used for changing processing of data,
  // it's intended purpose is to allow per-thread data buckets for output.
  virtual void ExecuteRange(TaskSetPartition range_, uint32_t threadnum_) = 0;

  // Set Size - usually the number of data items to be processed, see
  // ExecuteRange. Defaults to 1
  uint32_t m_SetSize = 1;

  // Min Range - Minimum size of of TaskSetPartition range when splitting a task
  // set into partitions. Designed for reducing scheduling overhead by
  // preventing set being divided up too small. Ranges passed to ExecuteRange
  // will *not* be a mulitple of this, only attempts to deliver range sizes
  // larger than this most of the time. This should be set to a value which
  // results in computation effort of at least 10k clock cycles to minimize task
  // scheduler overhead. NOTE: The last partition will be smaller than
  // m_MinRange if m_SetSize is not a multiple of m_MinRange. Also known as
  // grain size in literature.
  uint32_t m_MinRange = 1;

private:
  friend class TaskScheduler;
  void OnDependenciesComplete(TaskScheduler *pTaskScheduler_,
                              uint32_t threadNum_) override final;
...
public:
  ...
  IPinnedTask(uint32_t threadNum_)
      : threadNum(threadNum_) {} // default is to run a task on main thread
...
typedef std::function<void(TaskSetPartition range, uint32_t threadnum)>
    TaskSetFunction;
...
// LambdaPinnedTask - a utility pinned task for creating tasks based on
// std::func.
...
public:
  ...
  LambdaPinnedTask(uint32_t threadNum_, PinnedTaskFunction func_)
      : IPinnedTask(threadNum_), m_Function(func_) {}
...
public:
  ...
  ENKITS_API Dependency(const ICompletable *pDependencyTask_,
...
struct ProfilerCallbacks {
  ...
  ProfilerCallbackFunc
      waitForNewTaskSuspendStart; // thread suspended waiting for new tasks
  ProfilerCallbackFunc waitForNewTaskSuspendStop; // thread unsuspended
  ProfilerCallbackFunc
      waitForTaskCompleteStart; // thread waiting for task completion
  ProfilerCallbackFunc waitForTaskCompleteStop; // thread stopped waiting
  ProfilerCallbackFunc
      waitForTaskCompleteSuspendStart; // thread suspended waiting task
                                       // completion
  ProfilerCallbackFunc waitForTaskCompleteSuspendStop; // thread unsuspended
};

// Custom allocator, set in TaskSchedulerConfig. Also see
// ENKI_CUSTOM_ALLOC_FILE_AND_LINE for file_ and line_
typedef void *(*AllocFunc)(size_t align_, size_t size_, void *userData_,
                           const char *file_, int line_);
typedef void (*FreeFunc)(void *ptr_, size_t size_, void *userData_,
                         const char *file_, int line_);
ENKITS_API void *DefaultAllocFunc(size_t align_, size_t size_, void *userData_,
                                  const char *file_, int line_);
ENKITS_API void DefaultFreeFunc(void *ptr_, size_t size_, void *userData_,
                                const char *file_, int line_);
...
struct TaskSchedulerConfig {
  // numTaskThreadsToCreate - Number of tasking threads the task scheduler will
  // create. Must be > 0. Defaults to GetNumHardwareThreads()-1 threads as
  // thread which calls initialize is thread 0.
  uint32_t numTaskThreadsToCreate = GetNumHardwareThreads() - 1;

  // numExternalTaskThreads - Advanced use. Number of external threads which
  // need to use TaskScheduler API. See
  // TaskScheduler::RegisterExternalTaskThread() for usage. Defaults to 0. The
  // thread used to initialize the TaskScheduler can also use the TaskScheduler
  // API. Thus there are (numTaskThreadsToCreate + numExternalTaskThreads + 1)
  // able to use the API, with this defaulting to the number of harware threads
...
public:
  ...
  // Initialize() will create GetNumHardwareThreads()-1 tasking threads, which
  // is sufficient to fill the system when including the main thread. Initialize
  // can be called multiple times - it will wait for completion before
  // re-initializing.
  ...
  // Initialize with advanced TaskSchedulerConfig settings. See
  // TaskSchedulerConfig.
  ...
  // while( GetIsRunning() ) {} can be used in tasks which loop, to check if
  // enkiTS has been shutdown. If GetIsRunning() returns false should then exit.
  // Not required for finite tasks
  inline bool GetIsRunning() const {
    return m_bRunning.load(std::memory_order_acquire);
  }

  // while( !GetIsWaitforAllCalled() ) {} can be used in tasks which loop, to
  // check if WaitforAll() has been called. If GetIsWaitforAllCalled() returns
  // false should then exit. Not required for finite tasks This is intended to
  // be used with code which calls WaitforAll() with flag
  // WAITFORALLFLAGS_INC_WAIT_NEW_PINNED_TASKS set. This is also set when the
  // the task manager is shutting down, so no need to have an additional check
  // for GetIsRunning()
  ...
  // This function will run any IPinnedTask* for current thread, but not run
  // other Main thread should call this or use a wait to ensure it's tasks are
  // run.
  ENKITS_API void RunPinnedTasks();

  // Runs the TaskSets in pipe until true == pTaskSet->GetIsComplete();
  // should only be called from thread which created the taskscheduler , or
  // within a task if called with 0 it will try to run tasks, and return if none
  // available. To run only a subset of tasks, set priorityOfLowestToRun_ to a
  // high priority. Default is lowest priority available. Only wait for child
  // tasks of the current task otherwise a deadlock could occur.
  ENKITS_API void WaitforTask(const ICompletable *pCompletable_,
                              enki::TaskPriority priorityOfLowestToRun_ =
                                  TaskPriority(TASK_PRIORITY_NUM - 1));

  // Waits for all task sets to complete - not guaranteed to work unless we know
  // we are in a situation where tasks aren't being continuously added. If you
  // are running tasks which loop, make sure to check GetIsWaitforAllCalled()
  // and exit
  ENKITS_API void WaitforAll();

  // Waits for all task sets to complete and shutdown threads - not guaranteed
  // to work unless we know we are in a situation where tasks aren't being
  // continuously added. This function can be safely called even if
  // TaskScheduler::Initialize() has not been called.
  ...
  // Can be used with both ExternalTaskThreads or with an enkiTS tasking thread
  // to create a thread which only runs pinned tasks. If enkiTS threads are used
  // can create extra enkiTS task threads to handle non blocking computation via
  // normal tasks.
  ENKITS_API void WaitForNewPinnedTasks();

  // Returns the number of threads created for running tasks + number of
  // external threads plus 1 to account for the thread used to initialize the
  // task scheduler. Equivalent to config values: numTaskThreadsToCreate +
  // numExternalTaskThreads + 1. It is guaranteed that GetThreadNum() <
  // GetNumTaskThreads()
  ...
  // and all other non-enkiTS threads which have not been registered ( see
  // RegisterExternalTaskThread() ), and < GetNumTaskThreads() for all threads.
  ...
  // Intended for developers who have threads who need to call the TaskScheduler
  // API Returns true if successfull, false if not. Can only have
  // numExternalTaskThreads registered at any one time, which must be set at
  // initialization time.
  ENKITS_API bool RegisterExternalTaskThread();

  // As RegisterExternalTaskThread() but explicitly requests a given thread
  // number. threadNumToRegister_ must be  >= GetNumFirstExternalTaskThread()
  ...
  // Call on a thread on which RegisterExternalTaskThread has been called to
  // deregister that thread.
  ...
  // from GetNumFirstExternalTaskThread() up to (
  // GetNumFirstExternalTaskThread() + numExternalTaskThreads ) Note that if
  // numExternalTaskThreads == 0 a for loop using this will be valid: for(
  // uint32_t externalThreadNum = GetNumFirstExternalTaskThread();
  ...
  inline void WaitforTaskSet(const ICompletable *pCompletable_) {
    WaitforTask(pCompletable_);
  }
  ...
  inline ProfilerCallbacks *GetProfilerCallbacks() {
...
private:
  ...
  bool TryRunTask(uint32_t threadNum_, uint32_t priority_,
  ...
  ENKITS_API void TaskComplete(ICompletable *pTask_, bool bWakeThreads_,
                               uint32_t threadNum_);
...
inline void ICompletable::OnDependenciesComplete(TaskScheduler *pTaskScheduler_,
                                                 uint32_t threadNum_) {
  m_RunningCount.fetch_sub(1, std::memory_order_release);
  pTaskScheduler_->TaskComplete(this, true, threadNum_);
}

inline void ITaskSet::OnDependenciesComplete(TaskScheduler *pTaskScheduler_,
                                             uint32_t threadNum_) {
  pTaskScheduler_->AddTaskSetToPipeInt(this, threadNum_);
}

inline void IPinnedTask::OnDependenciesComplete(TaskScheduler *pTaskScheduler_,
                                                uint32_t threadNum_) {
  (void)threadNum_;
...
inline void ICompletable::SetDependency(Dependency &dependency_,
                                        const ICompletable *pDependencyTask_) {
...
template <typename D, typename T, int SIZE>
void ICompletable::SetDependenciesArr(D &dependencyArray_,
                                      const T (&taskArray_)[SIZE]) {
  static_assert(std::tuple_size<D>::value >= SIZE,
                "Size of dependency array too small");
...
template <typename D, typename T, int SIZE>
void ICompletable::SetDependenciesArr(D (&dependencyArray_)[SIZE],
                                      const T (&taskArray_)[SIZE]) {
  for (int i = 0; i < SIZE; ++i) {
    dependencyArray_[i].SetDependency(&taskArray_[i], this);
  }
...
template <typename D, typename T, int SIZE>
void ICompletable::SetDependenciesVec(D &dependencyVec_,
                                      const T (&taskArray_)[SIZE]) {
