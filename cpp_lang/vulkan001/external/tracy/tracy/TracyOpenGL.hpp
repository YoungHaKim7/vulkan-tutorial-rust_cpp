...
#define TracyGpuContext                                                        \
  tracy::GetGpuCtx().ptr =                                                     \
      (tracy::GpuCtx *)tracy::tracy_malloc(sizeof(tracy::GpuCtx));             \
  new (tracy::GetGpuCtx().ptr) tracy::GpuCtx;
#define TracyGpuContextName(name, size)                                        \
  tracy::GetGpuCtx().ptr->Name(name, size);
...
#define TracyGpuNamedZone(varname, name, active)                               \
  ...
  tracy::GpuCtxScope varname(                                                  \
      &TracyConcat(__tracy_gpu_source_location, __LINE__), TRACY_CALLSTACK,    \
      active);
...
#define TracyGpuZone(name)                                                     \
  TracyGpuNamedZoneS(___tracy_gpu_zone, name, TRACY_CALLSTACK, true)
...
#define TracyGpuZoneTransient(varname, name, active)                           \
  tracy::GpuCtxScope varname(__LINE__, __FILE__, strlen(__FILE__),             \
...
#define TracyGpuNamedZone(varname, name, active)                               \
  static constexpr tracy::SourceLocationData TracyConcat(                      \
      __tracy_gpu_source_location, __LINE__){name, __FUNCTION__, __FILE__,     \
                                             (uint32_t)__LINE__, 0};           \
  tracy::GpuCtxScope varname(                                                  \
      &TracyConcat(__tracy_gpu_source_location, __LINE__), active);
#define TracyGpuNamedZoneC(varname, name, color, active)                       \
  static constexpr tracy::SourceLocationData TracyConcat(                      \
      __tracy_gpu_source_location, __LINE__){name, __FUNCTION__, __FILE__,     \
                                             (uint32_t)__LINE__, color};       \
...
#define TracyGpuZoneC(name, color)                                             \
  TracyGpuNamedZoneC(___tracy_gpu_zone, name, color, true)
#define TracyGpuZoneTransient(varname, name, active)                           \
  tracy::GpuCtxScope varname(__LINE__, __FILE__, strlen(__FILE__),             \
                             __FUNCTION__, strlen(__FUNCTION__), name,         \
                             strlen(name), active);
...
#define TracyGpuNamedZoneS(varname, name, depth, active)                       \
  static constexpr tracy::SourceLocationData TracyConcat(                      \
      __tracy_gpu_source_location, __LINE__){name, __FUNCTION__, __FILE__,     \
                                             (uint32_t)__LINE__, 0};           \
  tracy::GpuCtxScope varname(                                                  \
      &TracyConcat(__tracy_gpu_source_location, __LINE__), depth, active);
#define TracyGpuNamedZoneCS(varname, name, color, depth, active)               \
  ...
  tracy::GpuCtxScope varname(                                                  \
      &TracyConcat(__tracy_gpu_source_location, __LINE__), depth, active);
#define TracyGpuZoneS(name, depth)                                             \
  TracyGpuNamedZoneS(___tracy_gpu_zone, name, depth, true)
#define TracyGpuZoneCS(name, color, depth)                                     \
  TracyGpuNamedZoneCS(___tracy_gpu_zone, name, color, depth, true)
...
#else
#define TracyGpuNamedZoneS(varname, name, depth, active)                       \
  TracyGpuNamedZone(varname, name, active)
#define TracyGpuNamedZoneCS(varname, name, color, depth, active)               \
...
#define TracyGpuZoneTransientS(varname, name, depth, active)                   \
  TracyGpuZoneTransient(varname, name, active)
#endif
...
public:
  ...
  void Collect() {
    ...
    while (m_tail != m_head) {
      GLint available;
      glGetQueryObjectiv(m_query[m_tail], GL_QUERY_RESULT_AVAILABLE,
                         &available);
      if (!available)
        return;
...
public:
...
#endif
  {
    ...
    glQueryCounter(GetGpuCtx().ptr->TranslateOpenGlQueryId(queryId),
                   GL_TIMESTAMP);
  ...
  tracy_force_inline GpuCtxScope(uint32_t line, const char *source,
                                 size_t sourceSz, const char *function,
                                 size_t functionSz, const char *name,
                                 size_t nameSz, bool is_active)
#ifdef TRACY_ON_DEMAND
      : m_active(is_active && GetProfiler().IsConnected())
#else
      : m_active(is_active)
  ...
  tracy_force_inline GpuCtxScope(uint32_t line, const char *source,
                                 size_t sourceSz, const char *function,
                                 size_t functionSz, const char *name,
                                 size_t nameSz, int depth, bool is_active)
#ifdef TRACY_ON_DEMAND
      : m_active(is_active && GetProfiler().IsConnected())
#else
      : m_active(is_active)
