...
static tracy_force_inline void SendLuaCallstack(lua_State *L, uint32_t depth) {
  ...
  spaceNeeded +=
      cnt *
      (4 + 2 + 2); // source line, function string length, source string length
...
static inline int LuaZoneBeginS(lua_State *L) {
  ...
  const auto srcloc = Profiler::AllocSourceLocation(
      dbg.currentline, dbg.source, dbg.name ? dbg.name : dbg.short_src);
...
static inline int LuaZoneBeginNS(lua_State *L) {
  ...
  const auto srcloc = Profiler::AllocSourceLocation(
      dbg.currentline, dbg.source, dbg.name ? dbg.name : dbg.short_src, name,
      nsz);
