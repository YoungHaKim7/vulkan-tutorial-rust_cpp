...
public:
  D3D11Ctx(ID3D11Device *device, ID3D11DeviceContext *devicectx)
      : m_device(device), m_devicectx(devicectx),
    ...
    for (int attempts = 0; attempts < 50; attempts++) {
      ...
      while (devicectx->GetData(m_disjoints[0], &disjoint, sizeof(disjoint),
                                0) == S_FALSE)
        /* Nothing */;
      ...
      while (devicectx->GetData(m_queries[0], &timestamp, sizeof(timestamp),
                                0) == S_FALSE)
        /* Nothing */;
  ...
  void Collect() {
    ...
    while (cnt > 1) {
      ...
      bool available =
          m_devicectx->GetData(m_disjointMap[mid % QueryCount], nullptr, 0,
                               D3D11_ASYNC_GETDATA_DONOTFLUSH) == S_OK &&
          m_devicectx->GetData(m_queries[mid % QueryCount], nullptr, 0,
                               D3D11_ASYNC_GETDATA_DONOTFLUSH) == S_OK;
    ...
    while (m_tail != start) {
      ...
      m_devicectx->GetData(m_disjointMap[m_tail], &disjoint, sizeof(disjoint),
                           0);
...
private:
  ...
  tracy_force_inline ID3D11Query *MapDisjointQueryId(unsigned int id,
                                                     unsigned int disjointId) {
  ...
  ID3D11Query *m_disjointMap[QueryCount]; // Multiple time queries can have one
                                          // disjoint query
  uint8_t m_context;
...
class D3D11ZoneScope {
public:
  tracy_force_inline D3D11ZoneScope(D3D11Ctx *ctx,
  ...
  tracy_force_inline
  D3D11ZoneScope(D3D11Ctx *ctx, uint32_t line, const char *source,
                 size_t sourceSz, const char *function, size_t functionSz,
                 const char *name, size_t nameSz, bool active)
#ifdef TRACY_ON_DEMAND
  ...
  D3D11ZoneScope(D3D11Ctx *ctx, uint32_t line, const char *source,
                 size_t sourceSz, const char *function, size_t functionSz,
                 const char *name, size_t nameSz, int depth, bool active)
#ifdef TRACY_ON_DEMAND
      : m_active(active && GetProfiler().IsConnected())
#else
      : m_active(active)
...
#define TracyD3D11Context(device, devicectx)                                   \
  tracy::CreateD3D11Context(device, devicectx);
...
#define TracyD3D11Zone(ctx, name)                                              \
  TracyD3D11NamedZoneS(ctx, ___tracy_gpu_zone, name, TRACY_CALLSTACK, true)
#define TracyD3D11ZoneC(ctx, name, color)                                      \
  TracyD3D11NamedZoneCS(ctx, ___tracy_gpu_zone, name, color, TRACY_CALLSTACK,  \
                        true)
#define TracyD3D11NamedZone(ctx, varname, name, active)                        \
...
#else
#define TracyD3D11Zone(ctx, name)                                              \
  TracyD3D11NamedZone(ctx, ___tracy_gpu_zone, name, true)
#define TracyD3D11ZoneC(ctx, name, color)                                      \
  TracyD3D11NamedZoneC(ctx, ___tracy_gpu_zone, name, color, true)
#define TracyD3D11NamedZone(ctx, varname, name, active)                        \
  static constexpr tracy::SourceLocationData TracyConcat(                      \
      __tracy_gpu_source_location, __LINE__){name, __FUNCTION__, __FILE__,     \
                                             (uint32_t)__LINE__, 0};           \
...
#define TracyD3D11NamedZoneC(ctx, varname, name, color, active)                \
  ...
  tracy::D3D11ZoneScope varname(                                               \
      ctx, &TracyConcat(__tracy_gpu_source_location, __LINE__), active);
#define TracyD3D11ZoneTransient(ctx, varname, name, active)                    \
  tracy::D3D11ZoneScope varname{ctx,          __LINE__,                        \
...
#define TracyD3D11ZoneS(ctx, name, depth)                                      \
  TracyD3D11NamedZoneS(ctx, ___tracy_gpu_zone, name, depth, true)
#define TracyD3D11ZoneCS(ctx, name, color, depth)                              \
  TracyD3D11NamedZoneCS(ctx, ___tracy_gpu_zone, name, color, depth, true)
#define TracyD3D11NamedZoneS(ctx, varname, name, depth, active)                \
  static constexpr tracy::SourceLocationData TracyConcat(                      \
      __tracy_gpu_source_location, __LINE__){name, __FUNCTION__, __FILE__,     \
                                             (uint32_t)__LINE__, 0};           \
...
#define TracyD3D11ZoneCS(ctx, name, color, depth, active)                      \
  TracyD3D11ZoneC(name, color)
#define TracyD3D11NamedZoneS(ctx, varname, name, depth, active)                \
  TracyD3D11NamedZone(ctx, varname, name, active)
#define TracyD3D11NamedZoneCS(ctx, varname, name, color, depth, active)        \
  TracyD3D11NamedZoneC(ctx, varname, name, color, active)
#define TracyD3D11ZoneTransientS(ctx, varname, name, depth, active)            \
  TracyD3D12ZoneTransient(ctx, varname, name, active)
#endif
