...
#define TRACY_CL_ASSERT(p)                                                     \
  if (!(p)) {                                                                  \
    TracyMessageL("TRACY_CL_ASSERT failed on " __FILE__                        \
                  ":" TRACY_CL_TO_STRING(__LINE__));                           \
...
public:
  ...
  OpenCLCtx(cl_context context, cl_device_id device)
    ...
    cl_mem dummyBuffer = clCreateBuffer(context, CL_MEM_WRITE_ONLY,
                                        sizeof(uint32_t), nullptr, &err);
    TRACY_CL_CHECK_ERROR(err)
    cl_event writeBufferEvent;
    TRACY_CL_CHECK_ERROR(clEnqueueWriteBuffer(queue, dummyBuffer, CL_FALSE, 0,
                                              sizeof(uint32_t), &dummyValue, 0,
    ...
    cl_int eventStatus;
    TRACY_CL_CHECK_ERROR(clGetEventInfo(writeBufferEvent,
                                        CL_EVENT_COMMAND_EXECUTION_STATUS,
                                        sizeof(cl_int), &eventStatus, nullptr));
    TRACY_CL_ASSERT(eventStatus == CL_COMPLETE);
    TRACY_CL_CHECK_ERROR(
        clGetEventProfilingInfo(writeBufferEvent, CL_PROFILING_COMMAND_END,
                                sizeof(cl_ulong), &tgpu, nullptr));
  ...
  void Collect() {
    ...
    for (; m_tail != m_head; m_tail = (m_tail + 1) % QueryCount) {
      ...
      cl_int eventStatus;
      cl_int err =
          clGetEventInfo(eventInfo.event, CL_EVENT_COMMAND_EXECUTION_STATUS,
                         sizeof(cl_int), &eventStatus, nullptr);
      if (err != CL_SUCCESS) {
        std::ostringstream oss;
        oss << "clGetEventInfo falied with error code " << err << ", on event "
            << eventInfo.event << ", skipping...";
        auto msg = oss.str();
        TracyMessage(msg.data(), msg.size());
        if (eventInfo.event == nullptr) {
          TracyMessageL("A TracyCLZone must be paird with a "
                        "TracyCLZoneSetEvent, check your code!");
        }
        assert(false && "clGetEventInfo failed, maybe a TracyCLZone is not "
                        "paired with TracyCLZoneSetEvent");
        continue;
      }
      ...
      err = clGetEventProfilingInfo(eventInfo.event, eventInfoQuery,
                                    sizeof(cl_ulong), &eventTimeStamp, nullptr);
      if (err == CL_PROFILING_INFO_NOT_AVAILABLE) {
        TracyMessageL("command queue is not created with "
                      "CL_QUEUE_PROFILING_ENABLE flag, check your code!");
        assert(
            false &&
            "command queue is not created with CL_QUEUE_PROFILING_ENABLE flag");
      } else
        TRACY_CL_CHECK_ERROR(err);
...
class OpenCLCtxScope {
public:
  tracy_force_inline OpenCLCtxScope(OpenCLCtx *ctx,
  ...
  tracy_force_inline OpenCLCtxScope(OpenCLCtx *ctx,
                                    const SourceLocationData *srcLoc, int depth,
                                    bool is_active)
#ifdef TRACY_ON_DEMAND
      : m_active(is_active && GetProfiler().IsConnected())
#else
  ...
  tracy_force_inline
  OpenCLCtxScope(OpenCLCtx *ctx, uint32_t line, const char *source,
                 size_t sourceSz, const char *function, size_t functionSz,
                 const char *name, size_t nameSz, bool is_active)
#ifdef TRACY_ON_DEMAND
      : m_active(is_active && GetProfiler().IsConnected())
#else
      : m_active(is_active)
  ...
  OpenCLCtxScope(OpenCLCtx *ctx, uint32_t line, const char *source,
                 size_t sourceSz, const char *function, size_t functionSz,
                 const char *name, size_t nameSz, int depth, bool is_active)
#ifdef TRACY_ON_DEMAND
      : m_active(is_active && GetProfiler().IsConnected())
#else
      : m_active(is_active)
  ...
  tracy_force_inline ~OpenCLCtxScope() {
    if (!m_active)
      return;
    const auto queryId =
        m_ctx->NextQueryId(EventInfo{m_event, EventPhase::End});
...
static inline OpenCLCtx *CreateCLContext(cl_context context,
                                         cl_device_id device) {
...
#define TracyCLNamedZone(ctx, varname, name, active)                           \
  ...
  tracy::OpenCLCtxScope varname(                                               \
      ctx, &TracyConcat(__tracy_gpu_source_location, __LINE__),                \
...
#define TracyCLZone(ctx, name)                                                 \
  TracyCLNamedZoneS(ctx, __tracy_gpu_zone, name, TRACY_CALLSTACK, true)
...
#define TracyCLZoneTransient(ctx, varname, name, active)                       \
  tracy::OpenCLCtxScope varname(ctx, __LINE__, __FILE__, strlen(__FILE__),     \
                                __FUNCTION__, strlen(__FUNCTION__), name,      \
                                strlen(name), TRACY_CALLSTACK, active);
#else
#define TracyCLNamedZone(ctx, varname, name, active)                           \
  static constexpr tracy::SourceLocationData TracyConcat(                      \
      __tracy_gpu_source_location, __LINE__){name, __FUNCTION__, __FILE__,     \
                                             (uint32_t)__LINE__, 0};           \
  tracy::OpenCLCtxScope varname(                                               \
      ctx, &TracyConcat(__tracy_gpu_source_location, __LINE__), active);
#define TracyCLNamedZoneC(ctx, varname, name, color, active)                   \
  static constexpr tracy::SourceLocationData TracyConcat(                      \
      __tracy_gpu_source_location, __LINE__){name, __FUNCTION__, __FILE__,     \
                                             (uint32_t)__LINE__, color};       \
  tracy::OpenCLCtxScope varname(                                               \
      ctx, &TracyConcat(__tracy_gpu_source_location, __LINE__), active);
#define TracyCLZone(ctx, name)                                                 \
  TracyCLNamedZone(ctx, __tracy_gpu_zone, name, true)
#define TracyCLZoneC(ctx, name, color)                                         \
  TracyCLNamedZoneC(ctx, __tracy_gpu_zone, name, color, true)
#define TracyCLZoneTransient(ctx, varname, name, active)                       \
  tracy::OpenCLCtxScope varname(ctx, __LINE__, __FILE__, strlen(__FILE__),     \
                                __FUNCTION__, strlen(__FUNCTION__), name,      \
                                strlen(name), active);
...
#define TracyCLNamedZoneS(ctx, varname, name, depth, active)                   \
  static constexpr tracy::SourceLocationData TracyConcat(                      \
      __tracy_gpu_source_location, __LINE__){name, __FUNCTION__, __FILE__,     \
                                             (uint32_t)__LINE__, 0};           \
  tracy::OpenCLCtxScope varname(                                               \
      ctx, &TracyConcat(__tracy_gpu_source_location, __LINE__), depth,         \
      active);
#define TracyCLNamedZoneCS(ctx, varname, name, color, depth, active)           \
  ...
  tracy::OpenCLCtxScope varname(                                               \
      ctx, &TracyConcat(__tracy_gpu_source_location, __LINE__), depth,         \
      active);
#define TracyCLZoneS(ctx, name, depth)                                         \
  TracyCLNamedZoneS(ctx, __tracy_gpu_zone, name, depth, true)
...
#define TracyCLZoneTransientS(ctx, varname, name, depth, active)               \
  tracy::OpenCLCtxScope varname(ctx, __LINE__, __FILE__, strlen(__FILE__),     \
                                __FUNCTION__, strlen(__FUNCTION__), name,      \
                                strlen(name), depth, active);
#else
#define TracyCLNamedZoneS(ctx, varname, name, depth, active)                   \
  TracyCLNamedZone(ctx, varname, name, active)
#define TracyCLNamedZoneCS(ctx, varname, name, color, depth, active)           \
