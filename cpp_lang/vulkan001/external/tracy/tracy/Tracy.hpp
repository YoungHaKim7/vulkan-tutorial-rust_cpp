...
#define ZoneNamedN(varname, name, active)                                      \
  static constexpr tracy::SourceLocationData TracyConcat(                      \
      __tracy_source_location, __LINE__){name, __FUNCTION__, __FILE__,         \
                                         (uint32_t)__LINE__, 0};               \
  tracy::ScopedZone varname(&TracyConcat(__tracy_source_location, __LINE__),   \
                            TRACY_CALLSTACK, active)
...
#define ZoneTransient(varname, active)                                         \
  tracy::ScopedZone varname(__LINE__, __FILE__, strlen(__FILE__),              \
                            __FUNCTION__, strlen(__FUNCTION__), nullptr, 0,    \
                            TRACY_CALLSTACK, active)
  ...
  tracy::Lockable<type> varname {                                              \
    []() -> const tracy::SourceLocationData * {                                \
      static constexpr tracy::SourceLocationData srcloc{                       \
          nullptr, #type " " #varname, __FILE__, __LINE__, 0};                 \
      return &srcloc;                                                          \
    }()                                                                        \
  }
#define TracyLockableN(type, varname, desc)                                    \
  tracy::Lockable<type> varname {                                              \
    []() -> const tracy::SourceLocationData * {                                \
      static constexpr tracy::SourceLocationData srcloc{                       \
          nullptr, desc, __FILE__, __LINE__, 0};                               \
      return &srcloc;                                                          \
    }()                                                                        \
  }
...
#define LockMark(varname)                                                      \
  static constexpr tracy::SourceLocationData __tracy_lock_location_##varname{  \
...
#define TracyMessage(txt, size)                                                \
  tracy::Profiler::Message(txt, size, TRACY_CALLSTACK)
...
#define TracyMessageLC(txt, color)                                             \
  tracy::Profiler::MessageColor(txt, color, TRACY_CALLSTACK)
...
#define TracySecureAllocN(ptr, size, name)                                     \
  tracy::Profiler::MemAllocCallstackNamed(ptr, size, TRACY_CALLSTACK, true,    \
                                          name)
...
#define TracyMessageC(txt, size, color)                                        \
  tracy::Profiler::MessageColor(txt, size, color, 0)
...
#define TracyAllocN(ptr, size, name)                                           \
  tracy::Profiler::MemAllocNamed(ptr, size, false, name)
#define TracyFreeN(ptr, name) tracy::Profiler::MemFreeNamed(ptr, false, name)
#define TracySecureAllocN(ptr, size, name)                                     \
  tracy::Profiler::MemAllocNamed(ptr, size, true, name)
#define TracySecureFreeN(ptr, name)                                            \
  tracy::Profiler::MemFreeNamed(ptr, true, name)
#endif
...
#define TracySecureFreeS(ptr, depth)                                           \
  tracy::Profiler::MemFreeCallstack(ptr, depth, true)
...
#define TracyMessageS(txt, size, depth)                                        \
  tracy::Profiler::Message(txt, size, depth)
...
#define TracyMessageLCS(txt, color, depth)                                     \
  tracy::Profiler::MessageColor(txt, color, depth)
#else
#define ZoneNamedS(varname, depth, active) ZoneNamed(varname, active)
#define ZoneNamedNS(varname, name, depth, active)                              \
  ZoneNamedN(varname, name, active)
#define ZoneNamedCS(varname, color, depth, active)                             \
  ZoneNamedC(varname, color, active)
#define ZoneNamedNCS(varname, name, color, depth, active)                      \
  ZoneNamedNC(varname, name, color, active)

#define ZoneTransientS(varname, depth, active) ZoneTransient(varname, active)
#define ZoneTransientNS(varname, name, depth, active)                          \
  ZoneTransientN(varname, name, active)
...
#define TracySourceCallbackRegister(cb, data)                                  \
  tracy::Profiler::SourceCallbackRegister(cb, data)
#define TracyParameterRegister(cb, data)                                       \
  tracy::Profiler::ParameterRegister(cb, data)
