...
public:
  VkCtx(VkPhysicalDevice physdev, VkDevice device, VkQueue queue,
      ...
      : m_device(device), m_timeDomain(VK_TIME_DOMAIN_DEVICE_EXT),
        m_context(GetGpuCtxCounter().fetch_add(1, std::memory_order_relaxed)),
        m_head(0), m_tail(0), m_oldCnt(0), m_queryCount(QueryCount),
        m_vkGetCalibratedTimestampsEXT(_vkGetCalibratedTimestampsEXT) {
    assert(m_context != 255);

    if (_vkGetPhysicalDeviceCalibrateableTimeDomainsEXT &&
    ...
    while (vkCreateQueryPool(device, &poolInfo, nullptr, &m_query) !=
           VK_SUCCESS) {
      m_queryCount /= 2;
      poolInfo.queryCount = m_queryCount;
    }
    ...
    if (m_timeDomain == VK_TIME_DOMAIN_DEVICE_EXT) {
      vkBeginCommandBuffer(cmdbuf, &beginInfo);
      vkCmdWriteTimestamp(cmdbuf, VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT, m_query,
                          0);
      vkEndCommandBuffer(cmdbuf);
      vkQueueSubmit(queue, 1, &submitInfo, VK_NULL_HANDLE);
      vkQueueWaitIdle(queue);
      ...
      vkGetQueryPoolResults(device, m_query, 0, 1, sizeof(tgpu), &tgpu,
                            sizeof(tgpu),
  ...
  void Collect(VkCommandBuffer cmdbuf) {
    ...
    if (vkGetQueryPoolResults(
            m_device, m_query, m_tail, cnt, sizeof(int64_t) * m_queryCount,
            m_res, sizeof(int64_t), VK_QUERY_RESULT_64_BIT) == VK_NOT_READY) {
      m_oldCnt = cnt;
      return;
    }
...
private:
  ...
  tracy_force_inline void Calibrate(VkDevice device, int64_t &tCpu,
                                    int64_t &tGpu) {
...
class VkCtxScope {
public:
  tracy_force_inline VkCtxScope(VkCtx *ctx, const SourceLocationData *srcloc,
                                VkCommandBuffer cmdbuf, bool is_active)
#ifdef TRACY_ON_DEMAND
  ...
  tracy_force_inline VkCtxScope(VkCtx *ctx, const SourceLocationData *srcloc,
                                VkCommandBuffer cmdbuf, int depth,
                                bool is_active)
#ifdef TRACY_ON_DEMAND
      : m_active(is_active && GetProfiler().IsConnected())
#else
  ...
  tracy_force_inline
  VkCtxScope(VkCtx *ctx, uint32_t line, const char *source, size_t sourceSz,
             const char *function, size_t functionSz, const char *name,
             size_t nameSz, VkCommandBuffer cmdbuf, bool is_active)
#ifdef TRACY_ON_DEMAND
      : m_active(is_active && GetProfiler().IsConnected())
#else
      : m_active(is_active)
  ...
  VkCtxScope(VkCtx *ctx, uint32_t line, const char *source, size_t sourceSz,
             const char *function, size_t functionSz, const char *name,
             size_t nameSz, VkCommandBuffer cmdbuf, int depth, bool is_active)
#ifdef TRACY_ON_DEMAND
      : m_active(is_active && GetProfiler().IsConnected())
#else
      : m_active(is_active)
...
static inline VkCtx *
CreateVkContext(VkPhysicalDevice physdev, VkDevice device, VkQueue queue,
                VkCommandBuffer cmdbuf,
...
#define TracyVkNamedZone(ctx, varname, cmdbuf, name, active)                   \
  ...
  tracy::VkCtxScope varname(                                                   \
      ctx, &TracyConcat(__tracy_gpu_source_location, __LINE__), cmdbuf,        \
...
#define TracyVkZoneC(ctx, cmdbuf, name, color)                                 \
  TracyVkNamedZoneCS(ctx, ___tracy_gpu_zone, cmdbuf, name, color,              \
...
#define TracyVkNamedZoneC(ctx, varname, cmdbuf, name, color, active)           \
  ...
  tracy::VkCtxScope varname(                                                   \
      ...
      active);
#define TracyVkZone(ctx, cmdbuf, name)                                         \
  TracyVkNamedZone(ctx, ___tracy_gpu_zone, cmdbuf, name, true)
#define TracyVkZoneC(ctx, cmdbuf, name, color)                                 \
  TracyVkNamedZoneC(ctx, ___tracy_gpu_zone, cmdbuf, name, color, true)
#define TracyVkZoneTransient(ctx, varname, cmdbuf, name, active)               \
  tracy::VkCtxScope varname(ctx, __LINE__, __FILE__, strlen(__FILE__),         \
                            __FUNCTION__, strlen(__FUNCTION__), name,          \
                            strlen(name), cmdbuf, active);
#endif
...
#define TracyVkNamedZoneS(ctx, varname, cmdbuf, name, depth, active)           \
  static constexpr tracy::SourceLocationData TracyConcat(                      \
      __tracy_gpu_source_location, __LINE__){name, __FUNCTION__, __FILE__,     \
                                             (uint32_t)__LINE__, 0};           \
  tracy::VkCtxScope varname(                                                   \
      ctx, &TracyConcat(__tracy_gpu_source_location, __LINE__), cmdbuf, depth, \
      active);
...
#define TracyVkZoneTransientS(ctx, varname, cmdbuf, name, depth, active)       \
  tracy::VkCtxScope varname(ctx, __LINE__, __FILE__, strlen(__FILE__),         \
                            __FUNCTION__, strlen(__FUNCTION__), name,          \
                            strlen(name), cmdbuf, depth, active);
#else
#define TracyVkNamedZoneS(ctx, varname, cmdbuf, name, depth, active)           \
