...
// This struct, as visible to user, is immutable, so treat it as if const was
// declared here.
...
TRACY_API void
___tracy_emit_gpu_zone_begin(const struct ___tracy_gpu_zone_begin_data);
...
TRACY_API void
___tracy_emit_gpu_context_name(const struct ___tracy_gpu_context_name_data);
TRACY_API void
___tracy_emit_gpu_calibration(const struct ___tracy_gpu_calibration_data);

TRACY_API void
___tracy_emit_gpu_zone_begin_serial(const struct ___tracy_gpu_zone_begin_data);
...
TRACY_API void ___tracy_emit_gpu_zone_begin_alloc_callstack_serial(
    const struct ___tracy_gpu_zone_begin_callstack_data);
TRACY_API void
___tracy_emit_gpu_zone_end_serial(const struct ___tracy_gpu_zone_end_data data);
TRACY_API void
___tracy_emit_gpu_time_serial(const struct ___tracy_gpu_time_data);
TRACY_API void ___tracy_emit_gpu_new_context_serial(
    const struct ___tracy_gpu_new_context_data);
TRACY_API void ___tracy_emit_gpu_context_name_serial(
    const struct ___tracy_gpu_context_name_data);
TRACY_API void ___tracy_emit_gpu_calibration_serial(
    const struct ___tracy_gpu_calibration_data);
...
#define TracyCZone(ctx, active)                                                \
  ...
  TracyCZoneCtx ctx = ___tracy_emit_zone_begin_callstack(                      \
      &TracyConcat(__tracy_source_location, __LINE__), TRACY_CALLSTACK,        \
      active);
#define TracyCZoneN(ctx, name, active)                                         \
  static const struct ___tracy_source_location_data TracyConcat(               \
      __tracy_source_location, __LINE__) = {name, __func__, __FILE__,          \
                                            (uint32_t)__LINE__, 0};            \
  TracyCZoneCtx ctx = ___tracy_emit_zone_begin_callstack(                      \
      &TracyConcat(__tracy_source_location, __LINE__), TRACY_CALLSTACK,        \
      active);
...
#define TracyCZone(ctx, active)                                                \
  ...
  TracyCZoneCtx ctx = ___tracy_emit_zone_begin(                                \
      &TracyConcat(__tracy_source_location, __LINE__), active);
#define TracyCZoneN(ctx, name, active)                                         \
  static const struct ___tracy_source_location_data TracyConcat(               \
      __tracy_source_location, __LINE__) = {name, __func__, __FILE__,          \
                                            (uint32_t)__LINE__, 0};            \
...
#define TracyCSecureFree(ptr)                                                  \
  ___tracy_emit_memory_free_callstack(ptr, TRACY_CALLSTACK, 1)

#define TracyCAllocN(ptr, size, name)                                          \
  ___tracy_emit_memory_alloc_callstack_named(ptr, size, TRACY_CALLSTACK, 0,    \
                                             name)
...
#define TracyCSecureAllocN(ptr, size, name)                                    \
  ___tracy_emit_memory_alloc_callstack_named(ptr, size, TRACY_CALLSTACK, 1,    \
                                             name)
...
#define TracyCSecureAllocN(ptr, size, name)                                    \
  ___tracy_emit_memory_alloc_named(ptr, size, 1, name);
#define TracyCSecureFreeN(ptr, name)                                           \
  ___tracy_emit_memory_free_named(ptr, 1, name);

#define TracyCMessage(txt, size) ___tracy_emit_message(txt, size, 0);
#define TracyCMessageL(txt) ___tracy_emit_messageL(txt, 0);
#define TracyCMessageC(txt, size, color)                                       \
  ___tracy_emit_messageC(txt, size, color, 0);
...
#define TracyCAllocS(ptr, size, depth)                                         \
  ___tracy_emit_memory_alloc_callstack(ptr, size, depth, 0)
#define TracyCFreeS(ptr, depth)                                                \
  ___tracy_emit_memory_free_callstack(ptr, depth, 0)
#define TracyCSecureAllocS(ptr, size, depth)                                   \
  ___tracy_emit_memory_alloc_callstack(ptr, size, depth, 1)
#define TracyCSecureFreeS(ptr, depth)                                          \
  ___tracy_emit_memory_free_callstack(ptr, depth, 1)
...
#define TracyCMessageS(txt, size, depth)                                       \
  ___tracy_emit_message(txt, size, depth);
#define TracyCMessageLS(txt, depth) ___tracy_emit_messageL(txt, depth);
#define TracyCMessageCS(txt, size, color, depth)                               \
  ___tracy_emit_messageC(txt, size, color, depth);
#define TracyCMessageLCS(txt, color, depth)                                    \
  ___tracy_emit_messageLC(txt, color, depth);
#else
...
#define TracyCSecureAllocNS(ptr, size, depth, name)                            \
  TracyCSecureAllocN(ptr, size, name)
