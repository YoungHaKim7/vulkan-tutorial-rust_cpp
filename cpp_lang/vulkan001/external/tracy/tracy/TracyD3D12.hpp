...
class D3D12QueueCtx {
  ...
  static constexpr uint32_t MaxQueries =
      64 * 1024; // Queries are begin and end markers, so we can store half as
                 // many total time durations. Must be even!
...
public:
  D3D12QueueCtx(ID3D12Device *device, ID3D12CommandQueue *queue)
    ...
    if (queue->GetDesc().Type == D3D12_COMMAND_LIST_TYPE_COPY) {
      ...
      bool Success = SUCCEEDED(device->CheckFeatureSupport(
          D3D12_FEATURE_D3D12_OPTIONS3, &featureData, sizeof(featureData)));
      assert(Success && featureData.CopyQueueTimestampQueriesSupported &&
             "Platform does not support profiling of copy queues.");
    }
    ...
    // Create a readback buffer, which will be used as a destination for the
    // query data.
    ...
    readbackBufferDesc.Layout =
        D3D12_TEXTURE_LAYOUT_ROW_MAJOR; // Buffers are always row major.
    ...
    MemWrite(&item->gpuNewContext.period,
             1E+09f / static_cast<float>(timestampFrequency));
  ...
  void NewFrame() {
    ...
    m_payloadQueue.emplace(
        D3D12QueryPayload{m_previousQueryCounter, queryCounter});
  ...
  void Collect() {
    ...
    const auto payloadCount =
        m_payloadQueue.size() - (m_activePayload - newestReadyPayload);
...
private:
  tracy_force_inline uint32_t NextQueryId() {
    ...
    assert(queryCounter < m_queryLimit &&
...
public:
  tracy_force_inline
  D3D12ZoneScope(D3D12QueueCtx *ctx, ID3D12GraphicsCommandList *cmdList,
...
#endif
  {
    ...
    cmdList->EndQuery(ctx->m_queryHeap.Get(), D3D12_QUERY_TYPE_TIMESTAMP,
                      m_queryId);

    auto *item = Profiler::QueueSerial();
    MemWrite(&item->hdr.type, QueueType::GpuZoneBeginSerial);
    MemWrite(&item->gpuZoneBegin.cpuTime, Profiler::GetTime());
    MemWrite(&item->gpuZoneBegin.srcloc,
             reinterpret_cast<uint64_t>(srcLocation));
  ...
  tracy_force_inline
  D3D12ZoneScope(D3D12QueueCtx *ctx, ID3D12GraphicsCommandList *cmdList,
                 const SourceLocationData *srcLocation, int depth, bool active)
#ifdef TRACY_ON_DEMAND
      : m_active(active && GetProfiler().IsConnected())
#else
      : m_active(active)
  ...
  tracy_force_inline
  D3D12ZoneScope(D3D12QueueCtx *ctx, uint32_t line, const char *source,
                 size_t sourceSz, const char *function, size_t functionSz,
...
#endif
  {
    ...
    const auto sourceLocation = Profiler::AllocSourceLocation(
        line, source, sourceSz, function, functionSz, name, nameSz);
...
#endif
  {
    ...
    auto *item = Profiler::QueueSerialCallstack(Callstack(depth));
    MemWrite(&item->hdr.type,
             QueueType::GpuZoneBeginAllocSrcLocCallstackSerial);
    MemWrite(&item->gpuZoneBegin.cpuTime, Profiler::GetTime());
    MemWrite(&item->gpuZoneBegin.srcloc, sourceLocation);
    MemWrite(&item->gpuZoneBegin.thread, GetThreadHandle());
    MemWrite(&item->gpuZoneBegin.queryId, static_cast<uint16_t>(m_queryId));
  ...
  tracy_force_inline ~D3D12ZoneScope() {
    ...
    const auto queryId =
        m_queryId +
        1; // Our end query slot is immediately after the begin slot.
    m_cmdList->EndQuery(m_ctx->m_queryHeap.Get(), D3D12_QUERY_TYPE_TIMESTAMP,
                        queryId);
    ...
    m_cmdList->ResolveQueryData(
        m_ctx->m_queryHeap.Get(), D3D12_QUERY_TYPE_TIMESTAMP, m_queryId, 2,
        m_ctx->m_readbackBuffer.Get(), m_queryId * sizeof(uint64_t));
  }
};

static inline D3D12QueueCtx *CreateD3D12Context(ID3D12Device *device,
...
#define TracyD3D12Context(device, queue)                                       \
  tracy::CreateD3D12Context(device, queue);
...
#define TracyD3D12Zone(ctx, cmdList, name)                                     \
  TracyD3D12NamedZoneS(ctx, ___tracy_gpu_zone, cmdList, name, TRACY_CALLSTACK, \
                       true)
#define TracyD3D12ZoneC(ctx, cmdList, name, color)                             \
  TracyD3D12NamedZoneCS(ctx, ___tracy_gpu_zone, cmdList, name, color,          \
                        TRACY_CALLSTACK, true)
#define TracyD3D12NamedZone(ctx, varname, cmdList, name, active)               \
  static constexpr tracy::SourceLocationData TracyConcat(                      \
      __tracy_gpu_source_location, __LINE__){name, __FUNCTION__, __FILE__,     \
                                             (uint32_t)__LINE__, 0};           \
  tracy::D3D12ZoneScope varname{                                               \
      ctx, cmdList, &TracyConcat(__tracy_gpu_source_location, __LINE__),       \
...
#else
#define TracyD3D12Zone(ctx, cmdList, name)                                     \
  TracyD3D12NamedZone(ctx, ___tracy_gpu_zone, cmdList, name, true)
#define TracyD3D12ZoneC(ctx, cmdList, name, color)                             \
  TracyD3D12NamedZoneC(ctx, ___tracy_gpu_zone, cmdList, name, color, true)
#define TracyD3D12NamedZone(ctx, varname, cmdList, name, active)               \
  static constexpr tracy::SourceLocationData TracyConcat(                      \
      __tracy_gpu_source_location, __LINE__){name, __FUNCTION__, __FILE__,     \
                                             (uint32_t)__LINE__, 0};           \
...
#define TracyD3D12NamedZoneC(ctx, varname, cmdList, name, color, active)       \
  ...
  tracy::D3D12ZoneScope varname{                                               \
      ...
      active};
#define TracyD3D12ZoneTransient(ctx, varname, cmdList, name, active)           \
  tracy::D3D12ZoneScope varname{ctx,          __LINE__,                        \
...
#define TracyD3D12ZoneS(ctx, cmdList, name, depth)                             \
  TracyD3D12NamedZoneS(ctx, ___tracy_gpu_zone, cmdList, name, depth, true)
#define TracyD3D12ZoneCS(ctx, cmdList, name, color, depth)                     \
  TracyD3D12NamedZoneCS(ctx, ___tracy_gpu_zone, cmdList, name, color, depth,   \
                        true)
#define TracyD3D12NamedZoneS(ctx, varname, cmdList, name, depth, active)       \
  static constexpr tracy::SourceLocationData TracyConcat(                      \
      __tracy_gpu_source_location, __LINE__){name, __FUNCTION__, __FILE__,     \
  ...
  tracy::D3D12ZoneScope varname{                                               \
      ctx, cmdList, &TracyConcat(__tracy_gpu_source_location, __LINE__),       \
      depth, active};
#define TracyD3D12NamedZoneCS(ctx, varname, cmdList, name, color, depth,       \
                              active)                                          \
  static constexpr tracy::SourceLocationData TracyConcat(                      \
      __tracy_gpu_source_location, __LINE__){name, __FUNCTION__, __FILE__,     \
                                             (uint32_t)__LINE__, color};       \
...
#define TracyD3D12ZoneCS(ctx, cmdList, name, color, depth)                     \
  TracyD3D12Zone(ctx, cmdList, name, color)
#define TracyD3D12NamedZoneS(ctx, varname, cmdList, name, depth, active)       \
