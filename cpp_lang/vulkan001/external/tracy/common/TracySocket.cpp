...
Socket::Socket()
    : m_buf((char *)tracy_malloc(BufSize)), m_bufPtr(nullptr), m_sock(-1),
      m_bufLeft(0), m_ptr(nullptr) {
#ifdef _WIN32
  InitWinSock();
#endif
}
...
Socket::Socket(int sock)
    : m_buf((char *)tracy_malloc(BufSize)), m_bufPtr(nullptr), m_sock(sock),
      m_bufLeft(0), m_ptr(nullptr) {}
...
static int addrinfo_and_socket_for_family(uint16_t port, int ai_family,
                                          struct addrinfo **res) {
  struct addrinfo hints;
  memset(&hints, 0, sizeof(hints));
...
#if defined _WIN32
  unsigned long val = 0;
  setsockopt(m_sock, IPPROTO_IPV6, IPV6_V6ONLY, (const char *)&val,
             sizeof(val));
#elif defined BSD
  int val = 0;
  setsockopt(m_sock, IPPROTO_IPV6, IPV6_V6ONLY, (const char *)&val,
...
int UdpBroadcast::Send(uint16_t port, const void *data, int len) {
  ...
  return sendto(m_sock, (const char *)data, len, MSG_NOSIGNAL,
                (sockaddr *)&addr, sizeof(addr));
}

IpAddress::IpAddress() : m_number(0) { *m_text = '\0'; }
...
bool UdpListen::Listen(uint16_t port) {
  ...
  setsockopt(m_sock, SOL_SOCKET, SO_REUSEADDR, (const char *)&reuse,
             sizeof(reuse));
#else
  int reuse = 1;
