...
void DBGHELP_UNLOCK;
    ...
    TRACY_HAS_CALLSTACK == 4 || TRACY_HAS_CALLSTACK == 5 ||                    \
    TRACY_HAS_CALLSTACK == 6
// If you want to use your own demangling functionality (e.g. for another
// language), define TRACY_DEMANGLE and provide your own implementation of the
// __tracy_demangle function. The input parameter is a function name. The
// demangle function must identify whether this name is mangled, and fail if it
// is not. Failure is indicated by returning nullptr. If demangling succeeds, a
// pointer to the C string containing demangled function must be returned. The
// demangling function is responsible for managing memory for this string. It is
// expected that it will be internally reused. When a call to ___tracy_demangle
// is made, previous contents of the string memory do not need to be preserved.
// Function may return string of any length, but the profiler can choose to
// truncate it.
...
void ___tracy_init_demangle_buffer() {
  ___tracy_demangle_buffer =
      (char *)tracy::tracy_malloc(___tracy_demangle_buffer_len);
}
...
extern "C" {
typedef DWORD(__stdcall *t_SymAddrIncludeInlineTrace)(HANDLE hProcess,
                                                      DWORD64 Address);
...
typedef BOOL(__stdcall *t_SymFromInlineContext)(HANDLE hProcess,
                                                DWORD64 Address,
                                                ULONG InlineContext,
                                                PDWORD64 Displacement,
                                                PSYMBOL_INFO Symbol);
typedef BOOL(__stdcall *t_SymGetLineFromInlineContext)(
    HANDLE hProcess, DWORD64 qwAddr, ULONG InlineContext,
...
void InitCallstackCritical() {
  ___tracy_RtlWalkFrameChain = (___tracy_t_RtlWalkFrameChain)GetProcAddress(
      GetModuleHandleA("ntdll.dll"), "RtlWalkFrameChain");
}

void InitCallstack() {
  _SymAddrIncludeInlineTrace = (t_SymAddrIncludeInlineTrace)GetProcAddress(
      GetModuleHandleA("dbghelp.dll"), "SymAddrIncludeInlineTrace");
  _SymQueryInlineTrace = (t_SymQueryInlineTrace)GetProcAddress(
      GetModuleHandleA("dbghelp.dll"), "SymQueryInlineTrace");
  _SymFromInlineContext = (t_SymFromInlineContext)GetProcAddress(
      GetModuleHandleA("dbghelp.dll"), "SymFromInlineContext");
  _SymGetLineFromInlineContext = (t_SymGetLineFromInlineContext)GetProcAddress(
      GetModuleHandleA("dbghelp.dll"), "SymGetLineFromInlineContext");
  ...
  if (EnumDeviceDrivers(dev, sizeof(dev), &needed) != 0) {
    ...
    for (size_t i = 0; i < sz; i++) {
      ...
      if (len != 0) {
        ...
        if (len != 0) {
          ...
          SymLoadModuleEx(GetCurrentProcess(), nullptr, path, nullptr,
                          (DWORD64)dev[i], 0, nullptr, 0);
    ...
    std::sort(s_krnlCache, s_krnlCache + s_krnlCacheCnt,
              [](const KernelDriver &lhs, const KernelDriver &rhs) {
                return lhs.addr > rhs.addr;
              });
  }

  s_modCache =
      (FastVector<ModuleCache> *)tracy_malloc(sizeof(FastVector<ModuleCache>));
...
const char *GetKernelModulePath(uint64_t addr) {
  ...
  auto it = std::lower_bound(s_krnlCache, s_krnlCache + s_krnlCacheCnt, addr,
                             [](const KernelDriver &lhs, const uint64_t &rhs) {
                               return lhs.addr > rhs;
                             });
...
static const char *GetModuleNameAndPrepareSymbols(uint64_t addr) {
  if ((addr >> 63) != 0) {
    if (s_krnlCache) {
      auto it =
          std::lower_bound(s_krnlCache, s_krnlCache + s_krnlCacheCnt, addr,
                           [](const KernelDriver &lhs, const uint64_t &rhs) {
                             return lhs.addr > rhs;
                           });
      if (it != s_krnlCache + s_krnlCacheCnt) {
    ...
    for (size_t i = 0; i < sz; i++) {
      ...
      if (GetModuleInformation(proc, mod[i], &info, sizeof(info)) != 0) {
        ...
        if (addr >= base && addr < base + info.SizeOfImage) {
          ...
          const auto res = GetModuleFileNameA(mod[i], name, 1021);
          if (res > 0) {
            // since this is the first time we encounter this module, load its
            // symbols (needed for modules loaded after SymInitialize)
            SymLoadModuleEx(proc, NULL, name, NULL, (DWORD64)info.lpBaseOfDll,
                            info.SizeOfImage, NULL, 0);
            auto ptr = name + res;
            while (ptr > name && *ptr != '\\' && *ptr != '/')
              ptr--;
            if (ptr > name)
...
CallstackEntryData DecodeCallstackPtr(uint64_t ptr) {
...

  {
    ...
    cb_data[write].name = symValid ? CopyStringFast(si->Name, si->NameLen)
                                   : CopyStringFast(moduleName);
  ...
  if (doInline) {
    for (DWORD i = 0; i < inlineNum; i++) {
      auto &cb = cb_data[i];
      const auto symInlineValid =
          _SymFromInlineContext(proc, ptr, ctx, nullptr, si) != 0;
      const char *filename;
      ...
      cb.name = symInlineValid ? CopyStringFast(si->Name, si->NameLen)
                               : CopyStringFast(moduleName);
...
static void InitKernelSymbols() {
  ...
  size_t linelen =
      16 * 1024; // linelen must be big enough to prevent reallocs in getline()
  ...
  std::sort(tmpSym.begin(), tmpSym.end(),
            [](const KernelSymbol &lhs, const KernelSymbol &rhs) {
              return lhs.addr > rhs.addr;
            });
  s_kernelSymCnt = tmpSym.size();
  s_kernelSym =
      (KernelSymbol *)tracy_malloc_fast(sizeof(KernelSymbol) * s_kernelSymCnt);
...
DebugInfo *FindDebugInfo(FastVector<DebugInfo> &vec,
                         const uint8_t *buildid_data, size_t buildid_size) {
  for (auto &v : vec) {
    if (v.buildid_size == buildid_size &&
        memcmp(v.buildid, buildid_data, buildid_size) == 0) {
      return &v;
    }
  }
  return nullptr;
...
int GetDebugInfoDescriptor(const char *buildid_data, size_t buildid_size,
  ...
  int fd =
      debuginfod_find_debuginfo(s_debuginfod, buildid, buildid_size, nullptr);
...
static int SymbolAddressDataCb(void *data, uintptr_t pc, uintptr_t lowaddr,
...
static void SymbolAddressErrorCb(void *data, const char * /*msg*/,
                                 int /*errnum*/) {
  auto &sym = *(CallstackSymbolData *)data;
  sym.file = "[unknown]";
  sym.line = 0;
...
CallstackSymbolData DecodeSymbolAddress(uint64_t ptr) {
  CallstackSymbolData sym;
  backtrace_pcinfo(cb_bts, ptr, SymbolAddressDataCb, SymbolAddressErrorCb,
                   &sym);
  return sym;
}

static int CallstackDataCb(void * /*data*/, uintptr_t pc, uintptr_t lowaddr,
                           const char *fn, int lineno, const char *function) {
  ...
  if (!fn && !function) {
    ...
    if (symoff == 0) {
      const auto len = std::min<size_t>(strlen(symname),
                                        std::numeric_limits<uint16_t>::max());
      cb_data[cb_num].name = CopyStringFast(symname, len);
    } else {
      char buf[32];
      const auto offlen = sprintf(buf, " + %td", symoff);
      const auto namelen = std::min<size_t>(
          strlen(symname), std::numeric_limits<uint16_t>::max() - offlen);
    ...
    const auto len = std::min<size_t>(strlen(function),
                                      std::numeric_limits<uint16_t>::max());
...
static void CallstackErrorCb(void * /*data*/, const char * /*msg*/,
                             int /*errnum*/) {
...
CallstackEntryData DecodeCallstackPtr(uint64_t ptr) {
  ...
  if (ptr >> 63 == 0) {
    ...
    auto it =
        std::lower_bound(s_kernelSym, s_kernelSym + s_kernelSymCnt, ptr,
...
CallstackEntryData DecodeCallstackPtr(uint64_t ptr) {
  ...
  if (symoff == 0) {
    const auto len =
        std::min<size_t>(strlen(symname), std::numeric_limits<uint16_t>::max());
