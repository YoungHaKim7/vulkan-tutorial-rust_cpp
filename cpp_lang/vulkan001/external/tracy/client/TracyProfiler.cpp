...
extern "C" typedef BOOL(WINAPI *t_GetLogicalProcessorInformationEx)(
    LOGICAL_PROCESSOR_RELATIONSHIP, PSYSTEM_LOGICAL_PROCESSOR_INFORMATION_EX,
    PDWORD);
#else
#include <limits.h>
#include <unistd.h>
...
// This is so far only needed on Android, where it is common for libraries to be
// mapped with only executable, not readable, permissions. Typical example (line
// from /proc/self/maps):
/*
746b63b000-746b6dc000 --xp 00042000 07:48 35
/apex/com.android.runtime/lib64/bionic/libc.so
*/
// See https://github.com/wolfpld/tracy/issues/125 .
// To work around this, we parse /proc/self/maps and we use mprotect to set read
// permissions on any mappings that contain symbols addresses hit by
// HandleSymbolCodeQuery.
...
// /proc/self/maps is assumed to be sorted by ascending address, so the
// resulting vector is sorted by ascending address too.
...
static MappingInfo *LookUpMapping(std::vector<MappingInfo> &mappings,
                                  uintptr_t address) {
  // Comparison function for std::lower_bound. Returns true if all addresses in
  // `m1` are lower than `addr`.
  ...
  auto iter =
      std::lower_bound(mappings.begin(), mappings.end(), address, Compare);
...
static MappingInfo *LookUpMapping(uintptr_t address) {
  // Static state managed by this function. Not constant, we mutate that state
  // as we turn some mappings readable. Initially parsed once here, updated as
  // needed below.
...
static bool EnsureReadable(MappingInfo &mapping) {
  ...
  // The mapping is now readable. Update `mapping` so the next call will be
  // fast.
  mapping.perm_r = true;
  return true;
}
...
#ifndef TRACY_UWP
    MessageBoxA(nullptr, msg, "Tracy Profiler initialization failure",
                MB_ICONSTOP);
#endif
  }
#else
...
static int64_t SetupHwTimer() {
  ...
  if (!CheckHardwareSupportsInvariantTSC()) {
    ...
    InitFailure("CPU doesn't support invariant TSC.\nDefine "
                "TRACY_NO_INVARIANT_CHECK=1 to ignore this error, *if you know "
                "what you are doing*.\nAlternatively you may rebuild the "
                "application with the TRACY_TIMER_QPC or TRACY_TIMER_FALLBACK "
                "define to use lower resolution timer.");
#else
    InitFailure(
        "CPU doesn't support invariant TSC.\nDefine TRACY_NO_INVARIANT_CHECK=1 "
        "to ignore this error, *if you know what you are "
        "doing*.\nAlternatively you may rebuild the application with the "
        "TRACY_TIMER_FALLBACK define to use lower resolution timer.");
#endif
  }
#endif
...
static const char *GetHostInfo() {
...
#else
  auto GetVersion = (t_RtlGetVersion)GetProcAddress(
...
#ifdef __MINGW32__
    ptr +=
        sprintf(ptr, "OS: Windows %i.%i.%i (MingW)\n", (int)ver.dwMajorVersion,
  ...
  if (fcpuinfo) {
    ...
    if (impl != 0 || var != 0 || part != 0 || rev != 0) {
      cpuFound = true;
      ptr += sprintf(ptr, "CPU: %s%s r%ip%i\n", DecodeArmImplementer(impl),
                     DecodeArmPart(impl, part), var, rev);
    }
  }
  if (!cpuFound) {
...
void Profiler::AckSymbolCodeNotAvailable() {
  ...
  AppendDataUnsafe(&item,
                   QueueDataSize[(int)QueueType::AckSymbolCodeNotAvailable]);
}

static BroadcastMessage &GetBroadcastMessage(const char *procname, size_t pnsz,
                                             int &len, int port) {
...
LONG WINAPI CrashFilter(PEXCEPTION_POINTERS pExp) {
  ...
  case EXCEPTION_ACCESS_VIOLATION:
    msgPtr +=
        sprintf(msgPtr, "Exception EXCEPTION_ACCESS_VIOLATION (0x%x). ", ec);
  ...
  case EXCEPTION_ARRAY_BOUNDS_EXCEEDED:
    msgPtr += sprintf(msgPtr,
                      "Exception EXCEPTION_ARRAY_BOUNDS_EXCEEDED (0x%x). ", ec);
    break;
  case EXCEPTION_DATATYPE_MISALIGNMENT:
    msgPtr += sprintf(msgPtr,
                      "Exception EXCEPTION_DATATYPE_MISALIGNMENT (0x%x). ", ec);
    break;
  case EXCEPTION_FLT_DIVIDE_BY_ZERO:
    msgPtr +=
        sprintf(msgPtr, "Exception EXCEPTION_FLT_DIVIDE_BY_ZERO (0x%x). ", ec);
    break;
  case EXCEPTION_ILLEGAL_INSTRUCTION:
    msgPtr +=
        sprintf(msgPtr, "Exception EXCEPTION_ILLEGAL_INSTRUCTION (0x%x). ", ec);
    break;
  ...
  case EXCEPTION_INT_DIVIDE_BY_ZERO:
    msgPtr +=
        sprintf(msgPtr, "Exception EXCEPTION_INT_DIVIDE_BY_ZERO (0x%x). ", ec);
    break;
  case EXCEPTION_PRIV_INSTRUCTION:
    msgPtr +=
        sprintf(msgPtr, "Exception EXCEPTION_PRIV_INSTRUCTION (0x%x). ", ec);
    break;
  case EXCEPTION_STACK_OVERFLOW:
    msgPtr +=
        sprintf(msgPtr, "Exception EXCEPTION_STACK_OVERFLOW (0x%x). ", ec);
    break;
  default:
    return EXCEPTION_CONTINUE_SEARCH;
  }
  ...
  do {
    if (te.th32OwnerProcessID == pid && te.th32ThreadID != tid &&
        te.th32ThreadID != s_profilerThreadId &&
...
#ifdef BUS_MCEERR_AR
    case BUS_MCEERR_AR:
      strcpy(msgPtr, "Hardware memory error consumed on a machine check; "
                     "action required.\n");
      break;
#endif
#ifdef BUS_MCEERR_AO
    case BUS_MCEERR_AO:
      strcpy(msgPtr, "Hardware memory error detected in process but not "
...
static ProfilerData &GetProfilerData() {
  ...
  if (!ptr) {
    int expected = 0;
    while (!profilerDataLock.compare_exchange_weak(
...
// GCC prior to 8.4 had a bug with function-inline thread_local variables.
// Versions of glibc beginning with 2.18 may attempt to work around this issue,
// which manifests as a crash while running static destructors if this function
// is compiled into a shared object. Unfortunately, centos7 ships with
// glibc 2.17. If running on old GCC, use the old-fashioned way as a workaround
...
// MSVC static initialization order solution. gcc/clang uses init_order() to
// avoid all this.

// 1a. But s_queue is needed for initialization of variables in point 2.
extern moodycamel::ConcurrentQueue<QueueItem> s_queue;

// 2. If these variables would be in the .CRT$XCB section, they would be
// initialized only in main thread.
thread_local moodycamel::ProducerToken init_order(107) s_token_detail(s_queue);
thread_local ProducerWrapper init_order(108) s_token{
    s_queue.get_explicit_producer(s_token_detail)};
thread_local ThreadHandleWrapper init_order(104) s_threadHandle{
...
struct ThreadNameData;
static std::atomic<ThreadNameData *> init_order(104)
    s_threadNameDataInstance(nullptr);
...
Profiler::Profiler()
    : m_timeBegin(0), m_mainThread(detail::GetThreadHandleImpl()),
      m_epoch(std::chrono::duration_cast<std::chrono::seconds>(
                  std::chrono::system_clock::now().time_since_epoch())
                  .count()),
      m_shutdown(false), m_shutdownManual(false), m_shutdownFinished(false),
      m_sock(nullptr), m_broadcast(nullptr), m_noExit(false), m_userPort(0),
      m_zoneId(1), m_samplingPeriod(0), m_stream(LZ4_createStream()),
      m_buffer((char *)tracy_malloc(TargetFrameSize * 3)), m_bufferOffset(0),
      m_bufferStart(0),
      m_lz4Buf((char *)tracy_malloc(LZ4Size + sizeof(lz4sz_t))),
      m_serialQueue(1024 * 1024), m_serialDequeue(1024 * 1024)
#ifndef TRACY_NO_FRAME_IMAGE
      ,
      m_fiQueue(16), m_fiDequeue(16)
#endif
      ,
      m_symbolQueue(8 * 1024), m_frameCount(0), m_isConnected(false)
#ifdef TRACY_ON_DEMAND
      ,
      m_connectionId(0), m_deferredQueue(64 * 1024)
#endif
      ,
      m_paramCallback(nullptr), m_sourceCallback(nullptr),
      m_queryImage(nullptr), m_queryData(nullptr),
...
#ifndef TRACY_DELAYED_INIT
#ifdef _MSC_VER
  // 3. But these variables need to be initialized in main thread within the
  // .CRT$XCB section. Do it here.
...
void Profiler::Worker() {
  ...
  const auto pnsz =
      std::min<size_t>(strlen(procname), WelcomeMessageProgramNameSize - 1);

  const auto hostinfo = GetHostInfo();
  const auto hisz =
      std::min<size_t>(strlen(hostinfo), WelcomeMessageHostInfoSize - 1);
  ...
  int broadcastLen = 0;
  auto &broadcastMsg =
      GetBroadcastMessage(procname, pnsz, broadcastLen, dataPort);
  uint64_t lastBroadcast = 0;

  // Connections loop.
  // Each iteration of the loop handles whole connection. Multiple iterations
  // will only happen in the on-demand mode or when handshake fails.
  for (;;) {
    ...
    // Handshake
    {
      ...
      if (!res || memcmp(shibboleth, HandshakeShibboleth,
                         HandshakeShibbolethSize) != 0) {
        m_sock->~Socket();
        tracy_free(m_sock);
        m_sock = nullptr;
        continue;
...
#ifndef TRACY_ON_DEMAND
    // Client is no longer available here. Accept incoming connections, but
    // reject handshake.
    for (;;) {
      if (ShouldExit()) {
        m_shutdownFinished.store(true, std::memory_order_relaxed);
  ...
  // Wait for symbols thread to terminate. Symbol resolution will continue in
  // this thread.
#ifdef TRACY_HAS_CALLSTACK
...
static void FreeAssociatedMemory(const QueueItem &item) {
  ...
  case QueueType::CallstackFrameSize: {
    ...
    auto data = (const CallstackEntry *)MemRead<uint64_t>(
        &item.callstackFrameSizeFat.data);
        ...
        while (sz-- > 0) {
          ...
          if (idx < (int)QueueType::Terminate) {
            ...
            case QueueType::FrameImage: {
              ...
              const auto csz = size_t(w * h / 2);
              SendLongString(ptr, (const char *)ptr, csz,
                             QueueType::FrameImageData);
              tracy_free_fast((void *)ptr);
              break;
            }
            ...
            case QueueType::CallstackFrameSize: {
              ...
              auto imageName = (const char *)MemRead<uint64_t>(
                  &item->callstackFrameSizeFat.imageName);
              ...
              for (uint8_t i = 0; i < datasz; i++) {
                ...
                AppendData(&item,
                           QueueDataSize[(int)QueueType::CallstackFrame]);
            ...
            case QueueType::SymbolInformation: {
              auto fileString = (const char *)MemRead<uint64_t>(
                  &item->symbolInformationFat.fileString);
              auto needFree =
                  MemRead<uint8_t>(&item->symbolInformationFat.needFree);
            ...
            case QueueType::ExternalNameMetadata: {
              auto thread =
                  MemRead<uint64_t>(&item->externalNameMetadata.thread);
              auto name = (const char *)MemRead<uint64_t>(
                  &item->externalNameMetadata.name);
              auto threadName = (const char *)MemRead<uint64_t>(
                  &item->externalNameMetadata.threadName);
...
Profiler::DequeueContextSwitches(tracy::moodycamel::ConsumerToken &token,
                                 int64_t &timeStop) {
  const auto sz = GetQueue().try_dequeue_bulk_single(
      token, [](const uint64_t &) {},
      [this, &timeStop](QueueItem *item, size_t sz) {
        ...
        while (sz-- > 0) {
          ...
          if (idx == (uint8_t)QueueType::ContextSwitch) {
            ...
            MemWrite(&item->contextSwitch.time, dt);
            if (!AppendData(item,
                            QueueDataSize[(int)QueueType::ContextSwitch])) {
              timeStop = -2;
              m_refTimeCtx = refCtx;
              return;
            }
            ...
            MemWrite(&item->threadWakeup.time, dt);
            if (!AppendData(item,
                            QueueDataSize[(int)QueueType::ThreadWakeup])) {
              timeStop = -2;
              m_refTimeCtx = refCtx;
              return;
            }
  ...
  if (timeStop == -2)
    return DequeueStatus::ConnectionLost;
  return (timeStop == -1 || sz > 0) ? DequeueStatus::DataDequeued
...
bool Profiler::SendData(const char *data, size_t len) {
  const lz4sz_t lz4sz = LZ4_compress_fast_continue(
      (LZ4_stream_t *)m_stream, data, m_lz4Buf + sizeof(lz4sz_t), (int)len,
      LZ4Size, 1);
  memcpy(m_lz4Buf, &lz4sz, sizeof(lz4sz));
  return m_sock->Send(m_lz4Buf, lz4sz + sizeof(lz4sz_t)) != -1;
}

void Profiler::SendString(uint64_t str, const char *ptr, size_t len,
                          QueueType type) {
  assert(type == QueueType::StringData || type == QueueType::ThreadName ||
         type == QueueType::PlotName || type == QueueType::FrameName ||
         type == QueueType::ExternalName ||
         type == QueueType::ExternalThreadName || type == QueueType::FiberName);
...
void Profiler::SendSingleString(const char *ptr, size_t len) {
  ...
  NeedDataSize(QueueDataSize[(int)QueueType::SingleStringData] + sizeof(l16) +
               l16);
...
void Profiler::SendSecondString(const char *ptr, size_t len) {
  ...
  NeedDataSize(QueueDataSize[(int)QueueType::SecondStringData] + sizeof(l16) +
               l16);
...
void Profiler::SendLongString(uint64_t str, const char *ptr, size_t len,
                              QueueType type) {
  assert(type == QueueType::FrameImageData || type == QueueType::SymbolCode ||
...
void Profiler::SendSourceLocationPayload(uint64_t _ptr) {
  ...
  NeedDataSize(QueueDataSize[(int)QueueType::SourceLocationPayload] +
               sizeof(len) + len);
...
void Profiler::SendCallstackPayload(uint64_t _ptr) {
  ...
  NeedDataSize(QueueDataSize[(int)QueueType::CallstackPayload] + sizeof(l16) +
               l16);
...
void Profiler::SendCallstackAlloc(uint64_t _ptr) {
  ...
  NeedDataSize(QueueDataSize[(int)QueueType::CallstackAllocPayload] +
               sizeof(len) + len);
...
void Profiler::QueueCallstackFrame(uint64_t ptr) {
  ...
  m_symbolQueue.emplace(
      SymbolQueueItem{SymbolQueueItemType::CallstackFrame, ptr});
#else
  AckServerQuery();
#endif
}
...
void Profiler::QueueSymbolQuery(uint64_t symbol) {
  ...
  if (symbol >> 63 != 0) {
    ...
    m_symbolQueue.emplace(
        SymbolQueueItem{SymbolQueueItemType::SymbolQuery, symbol});
  }
#else
  AckServerQuery();
#endif
...
#ifdef TRACY_HAS_SYSTEM_TRACING
  m_symbolQueue.emplace(
      SymbolQueueItem{SymbolQueueItemType::ExternalName, ptr});
#endif
}
...
#ifdef TRACY_HAS_CALLSTACK
  m_symbolQueue.emplace(
      SymbolQueueItem{SymbolQueueItemType::KernelCode, symbol, size});
#else
...
void Profiler::QueueSourceCodeQuery(uint32_t id) {
  assert(m_exectime != 0);
  assert(m_queryData);
  m_symbolQueue.emplace(SymbolQueueItem{SymbolQueueItemType::SourceCode,
                                        uint64_t(m_queryData),
                                        uint64_t(m_queryImage), id});
  m_queryData = nullptr;
...
void Profiler::HandleSymbolQueueItem(const SymbolQueueItem &si) {
  ...
  case SymbolQueueItemType::CallstackFrame: {
    ...
    MemWrite(&item->callstackFrameSizeFat.imageName,
             (uint64_t)frameData.imageName);
    TracyLfqCommit;
    break;
  }
  case SymbolQueueItemType::SymbolQuery: {
#ifdef __ANDROID__
    // On Android it's common for code to be in mappings that are only
...
void Profiler::CalibrateDelay() {
  ...
  static_assert(Events < QueuePrealloc,
                "Delay calibration loop will allocate memory in queue");

  static const tracy::SourceLocationData __tracy_source_location{
      nullptr, __FUNCTION__, __FILE__, (uint32_t)__LINE__, 0};
  ...
  while (left != 0) {
    const auto sz = GetQueue().try_dequeue_bulk_single(
        token, [](const uint64_t &) {}, [](QueueItem *item, size_t sz) {});
    assert(sz > 0);
    left -= (int)sz;
  }
...
void Profiler::ReportTopology() {
...
#else
  t_GetLogicalProcessorInformationEx _GetLogicalProcessorInformationEx =
      (t_GetLogicalProcessorInformationEx)GetProcAddress(
          GetModuleHandleA("kernel32.dll"), "GetLogicalProcessorInformationEx");
#endif
  ...
  auto packageInfo =
      (SYSTEM_LOGICAL_PROCESSOR_INFORMATION_EX *)tracy_malloc(psz);
  auto res = _GetLogicalProcessorInformationEx(RelationProcessorPackage,
                                               packageInfo, &psz);
  assert(res);
  ...
  res =
      _GetLogicalProcessorInformationEx(RelationProcessorCore, coreInfo, &csz);
  assert(res);
...
void Profiler::HandleSourceCodeQuery(char *data, char *image, uint32_t id) {
  ...
  if (stat(data, &st) == 0 && (uint64_t)st.st_mtime < m_exectime) {
    ...
    if (buildid) {
      auto d = debuginfod_find_source(GetDebuginfodClient(), buildid, size,
                                      data, nullptr);
...
TRACY_API TracyCZoneCtx ___tracy_emit_zone_begin_alloc(uint64_t srcloc,
                                                       int active) {
  ___tracy_c_zone_context ctx;
#ifdef TRACY_ON_DEMAND
  ctx.active = active && tracy::GetProfiler().IsConnected();
...
TRACY_API TracyCZoneCtx ___tracy_emit_zone_begin_alloc_callstack(
    uint64_t srcloc, int depth, int active) {
  ___tracy_c_zone_context ctx;
#ifdef TRACY_ON_DEMAND
  ctx.active = active && tracy::GetProfiler().IsConnected();
#else
...
TRACY_API void ___tracy_emit_zone_text(TracyCZoneCtx ctx, const char *txt,
                                       size_t size) {
...
TRACY_API void ___tracy_emit_zone_name(TracyCZoneCtx ctx, const char *txt,
                                       size_t size) {
  assert(size < std::numeric_limits<uint16_t>::max());
  if (!ctx.active)
    return;
...
TRACY_API void ___tracy_emit_memory_alloc_callstack(const void *ptr,
...
TRACY_API void ___tracy_emit_memory_alloc_callstack_named(const void *ptr,
                                                          size_t size,
                                                          int depth, int secure,
                                                          const char *name) {
...
TRACY_API void ___tracy_emit_memory_free_callstack_named(const void *ptr,
                                                         int depth, int secure,
                                                         const char *name) {
...
TRACY_API void ___tracy_emit_frame_image(const void *image, uint16_t w,
                                         uint16_t h, uint8_t offset, int flip) {
...
TRACY_API void ___tracy_emit_messageC(const char *txt, size_t size,
                                      uint32_t color, int callstack) {
...
TRACY_API uint64_t ___tracy_alloc_srcloc(uint32_t line, const char *source,
                                         size_t sourceSz, const char *function,
                                         size_t functionSz) {
  return tracy::Profiler::AllocSourceLocation(line, source, sourceSz, function,
                                              functionSz);
}

TRACY_API uint64_t ___tracy_alloc_srcloc_name(uint32_t line, const char *source,
                                              size_t sourceSz,
                                              const char *function,
                                              size_t functionSz,
                                              const char *name, size_t nameSz) {
  return tracy::Profiler::AllocSourceLocation(line, source, sourceSz, function,
                                              functionSz, name, nameSz);
}

TRACY_API void
___tracy_emit_gpu_zone_begin(const struct ___tracy_gpu_zone_begin_data data) {
...
TRACY_API void ___tracy_emit_gpu_zone_begin_callstack(
    const struct ___tracy_gpu_zone_begin_callstack_data data) {
...
TRACY_API void
___tracy_emit_gpu_time(const struct ___tracy_gpu_time_data data) {
...
TRACY_API void
___tracy_emit_gpu_zone_end(const struct ___tracy_gpu_zone_end_data data) {
...
TRACY_API void
___tracy_emit_gpu_new_context(___tracy_gpu_new_context_data data) {
...
TRACY_API void ___tracy_emit_gpu_context_name(
    const struct ___tracy_gpu_context_name_data data) {
...
TRACY_API void
___tracy_emit_gpu_calibration(const struct ___tracy_gpu_calibration_data data) {
...
TRACY_API void ___tracy_emit_gpu_zone_begin_serial(
    const struct ___tracy_gpu_zone_begin_data data) {
...
TRACY_API void ___tracy_emit_gpu_zone_begin_callstack_serial(
    const struct ___tracy_gpu_zone_begin_callstack_data data) {
  auto item =
      tracy::Profiler::QueueSerialCallstack(tracy::Callstack(data.depth));
...
TRACY_API void
___tracy_emit_gpu_time_serial(const struct ___tracy_gpu_time_data data) {
  auto item = tracy::Profiler::QueueSerial();
...
TRACY_API void
___tracy_emit_gpu_new_context_serial(___tracy_gpu_new_context_data data) {
  auto item = tracy::Profiler::QueueSerial();
...
TRACY_API void ___tracy_emit_gpu_context_name_serial(
    const struct ___tracy_gpu_context_name_data data) {
  auto ptr = (char *)tracy::tracy_malloc(data.len);
  memcpy(ptr, data.name, data.len);
...
TRACY_API void ___tracy_emit_gpu_calibration_serial(
    const struct ___tracy_gpu_calibration_data data) {
  auto item = tracy::Profiler::QueueSerial();
