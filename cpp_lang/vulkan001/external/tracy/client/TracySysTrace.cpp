...
#if defined _WIN32
  return TRACY_SAMPLING_HZ > 8000
             ? 8000
             : (TRACY_SAMPLING_HZ < 1 ? 1 : TRACY_SAMPLING_HZ);
#else
  return TRACY_SAMPLING_HZ > 1000000
             ? 1000000
             : (TRACY_SAMPLING_HZ < 1 ? 1 : TRACY_SAMPLING_HZ);
#endif
}
...
static const GUID DxgKrnlGuid = {
    0x802ec45a,
    0x1e99,
    0x4b83,
    {0x99, 0x20, 0x87, 0xc9, 0x82, 0x77, 0xba, 0x9d}};
static const GUID ThreadV2Guid = {
    0x3d6fa8d1,
    0xfe05,
    0x11d0,
...
extern "C" typedef NTSTATUS(WINAPI *t_NtQueryInformationThread)(HANDLE,
                                                                THREADINFOCLASS,
                                                                PVOID, ULONG,
                                                                PULONG);
extern "C" typedef BOOL(WINAPI *t_EnumProcessModules)(HANDLE, HMODULE *, DWORD,
                                                      LPDWORD);
extern "C" typedef BOOL(WINAPI *t_GetModuleInformation)(HANDLE, HMODULE,
                                                        LPMODULEINFO, DWORD);
extern "C" typedef DWORD(WINAPI *t_GetModuleBaseNameA)(HANDLE, HMODULE, LPSTR,
                                                       DWORD);
extern "C" typedef HRESULT(WINAPI *t_GetThreadDescription)(HANDLE, PWSTR *);

t_NtQueryInformationThread NtQueryInformationThread =
    (t_NtQueryInformationThread)GetProcAddress(GetModuleHandleA("ntdll.dll"),
                                               "NtQueryInformationThread");
t_EnumProcessModules _EnumProcessModules = (t_EnumProcessModules)GetProcAddress(
    GetModuleHandleA("kernel32.dll"), "K32EnumProcessModules");
t_GetModuleInformation _GetModuleInformation =
    (t_GetModuleInformation)GetProcAddress(GetModuleHandleA("kernel32.dll"),
                                           "K32GetModuleInformation");
t_GetModuleBaseNameA _GetModuleBaseNameA = (t_GetModuleBaseNameA)GetProcAddress(
    GetModuleHandleA("kernel32.dll"), "K32GetModuleBaseNameA");
...
void WINAPI EventRecordCallback(PEVENT_RECORD record) {
  ...
  case 0x3d6fa8d1: // Thread Guid
    if (hdr.EventDescriptor.Opcode == 36) {
      ...
      TracyLfqCommit;
    } else if (hdr.EventDescriptor.Opcode == 1 ||
               hdr.EventDescriptor.Opcode == 3) {
...
static void SetupVsync() {
  ...
  const auto controlStatus =
      ControlTraceA(0, "TracyVsync", s_propVsync, EVENT_TRACE_CONTROL_STOP);
  if (controlStatus != ERROR_SUCCESS &&
      controlStatus != ERROR_WMI_INSTANCE_NOT_FOUND) {
    tracy_free(backup);
  ...
  const auto startStatus =
      StartTraceA(&s_traceHandleVsync, "TracyVsync", s_propVsync);
  ...
  uint64_t mask =
      0x4000000000000001; // Microsoft_Windows_DxgKrnl_Performance | Base
  if (EnableTraceEx2(
          s_traceHandleVsync, &DxgKrnlGuid, EVENT_CONTROL_CODE_ENABLE_PROVIDER,
          TRACE_LEVEL_INFORMATION, mask, mask, 0, &params) != ERROR_SUCCESS) {
    tracy_free(s_propVsync);
    return;
  }
...
static constexpr int GetSamplingInterval() { return GetSamplingPeriod() / 100; }

bool SysTraceStart(int64_t &samplingPeriod) {
  if (!_GetThreadDescription)
    _GetThreadDescription = (t_GetThreadDescription)GetProcAddress(
  ...
  if (LookupPrivilegeValue(nullptr, SE_SYSTEM_PROFILE_NAME,
  ...
  if (OpenProcessToken(GetCurrentProcess(), TOKEN_ADJUST_PRIVILEGES, &pt) == 0)
    return false;
  const auto adjust =
      AdjustTokenPrivileges(pt, FALSE, &priv, 0, nullptr, nullptr);
  CloseHandle(pt);
  if (adjust == 0)
    return false;
  ...
  if (isOs64Bit) {
    ...
    const auto intervalStatus = TraceSetInformation(
        0, TraceSampledProfileIntervalInfo, &interval, sizeof(interval));
  ...
  flags = EVENT_TRACE_FLAG_CSWITCH | EVENT_TRACE_FLAG_DISPATCHER |
  ...
  memcpy(((char *)s_prop) + sizeof(EVENT_TRACE_PROPERTIES), KERNEL_LOGGER_NAME,
  ...
  const auto startStatus =
      StartTrace(&s_traceHandle, KERNEL_LOGGER_NAME, s_prop);
  if (startStatus != ERROR_SUCCESS) {
    tracy_free(s_prop);
    return false;
  }
  ...
  if (isOs64Bit) {
    ...
    stackId[1].Type = 36;
    const auto stackStatus = TraceSetInformation(
        s_traceHandle, TraceStackTracingInfo, &stackId, sizeof(stackId));
...
void SysTraceGetExternalName(uint64_t thread, const char *&threadName,
                             const char *&name) {
  ...
  if (hnd != 0) {
    ...
    if (!threadSent && NtQueryInformationThread && _EnumProcessModules &&
        _GetModuleInformation && _GetModuleBaseNameA) {
      void *ptr;
      ULONG retlen;
      auto status = NtQueryInformationThread(
          hnd, (THREADINFOCLASS)9 /*ThreadQuerySetWin32StartAddress*/, &ptr,
          sizeof(&ptr), &retlen);
      if (status == 0) {
        const auto phnd = OpenProcess(
            PROCESS_QUERY_INFORMATION | PROCESS_VM_READ, FALSE, pid);
        if (phnd != INVALID_HANDLE_VALUE) {
          HMODULE modules[1024];
          DWORD needed;
          if (_EnumProcessModules(phnd, modules, 1024 * sizeof(HMODULE),
                                  &needed) != 0) {
            const auto sz =
                std::min(DWORD(needed / sizeof(HMODULE)), DWORD(1024));
            for (DWORD i = 0; i < sz; i++) {
              MODULEINFO info;
              if (_GetModuleInformation(phnd, modules[i], &info,
                                        sizeof(info)) != 0) {
                if ((uint64_t)ptr >= (uint64_t)info.lpBaseOfDll &&
                    (uint64_t)ptr <= (uint64_t)info.lpBaseOfDll +
                                         (uint64_t)info.SizeOfImage) {
                  char buf2[1024];
                  const auto modlen =
                      _GetModuleBaseNameA(phnd, modules[i], buf2, 1024);
                  if (modlen != 0) {
...
static void ProbePreciseIp(perf_event_attr &pe, unsigned long long config0,
                           unsigned long long config1, pid_t pid) {
  pe.config = config1;
  pe.precise_ip = 3;
...
bool SysTraceStart(int64_t &samplingPeriod) {
  ...
  const auto paranoidLevelStr =
      ReadFile("/proc/sys/kernel/perf_event_paranoid");
  if (!paranoidLevelStr)
    return false;
#ifdef TRACY_VERBOSE
  int paranoidLevel = 2;
  ...
  const auto wakeupIdStr =
      ReadFile("/sys/kernel/debug/tracing/events/sched/sched_wakeup/id");
  if (wakeupIdStr)
    wakeupId = atoi(wakeupIdStr);
  const auto vsyncIdStr =
      ReadFile("/sys/kernel/debug/tracing/events/drm/drm_vblank_event/id");
  if (vsyncIdStr)
  ...
  const auto maxNumBuffers =
      s_numCpus * (1 + // software sampling
  ...
  pe.freq = 1;
  pe.inherit = 1;
#if !defined TRACY_HW_TIMER || !(defined __i386 || defined _M_IX86 ||          \
  ...
  if (!noRetirement) {
    ...
    for (int i = 0; i < s_numCpus; i++) {
      const int fd =
          perf_event_open(&pe, currentPid, i, -1, PERF_FLAG_FD_CLOEXEC);
      if (fd != -1) {
    ...
    for (int i = 0; i < s_numCpus; i++) {
      const int fd =
          perf_event_open(&pe, currentPid, i, -1, PERF_FLAG_FD_CLOEXEC);
      if (fd != -1) {
        new (s_ring + s_numBuffers)
            RingBuffer(64 * 1024, fd, EventInstructionsRetired);
        if (s_ring[s_numBuffers].IsValid()) {
          s_numBuffers++;
          TracyDebug("  Core %i ok\n", i);
        }
  ...
  if (!noCache) {
    ...
    ProbePreciseIp(pe, PERF_COUNT_HW_CACHE_REFERENCES,
                   PERF_COUNT_HW_CACHE_MISSES, currentPid);
    if (IsGenuineIntel()) {
      pe.precise_ip = 0;
    ...
    for (int i = 0; i < s_numCpus; i++) {
      ...
      if (fd != -1) {
        new (s_ring + s_numBuffers)
            RingBuffer(64 * 1024, fd, EventCacheReference);
        if (s_ring[s_numBuffers].IsValid()) {
          s_numBuffers++;
          TracyDebug("  Core %i ok\n", i);
        }
  ...
  if (!noBranch) {
    ...
    ProbePreciseIp(pe, PERF_COUNT_HW_BRANCH_INSTRUCTIONS,
                   PERF_COUNT_HW_BRANCH_MISSES, currentPid);
    for (int i = 0; i < s_numCpus; i++) {
      const int fd =
          perf_event_open(&pe, currentPid, i, -1, PERF_FLAG_FD_CLOEXEC);
      if (fd != -1) {
        new (s_ring + s_numBuffers)
            RingBuffer(64 * 1024, fd, EventBranchRetired);
        if (s_ring[s_numBuffers].IsValid()) {
          s_numBuffers++;
          TracyDebug("  Core %i ok\n", i);
        }
  ...
  if (!noCtxSwitch && switchId != -1) {
    ...
    for (int i = 0; i < s_numCpus; i++) {
      ...
      if (fd != -1) {
        new (s_ring + s_numBuffers)
            RingBuffer(256 * 1024, fd, EventContextSwitch, i);
        if (s_ring[s_numBuffers].IsValid()) {
          s_numBuffers++;
          TracyDebug("  Core %i ok\n", i);
        }
...
static uint64_t *GetCallstackBlock(uint64_t cnt, RingBuffer &ring,
                                   uint64_t offset) {
      ...
      if (activeNum > 0) {
        ...
        while (activeNum > 0) {
          ...
          for (int i = 0; i < activeNum; i++) {
            ...
            if (hdr.type == PERF_RECORD_SAMPLE) {
              int64_t rbTime;
              ringArray[rbIdx].Read(&rbTime, rbPos + sizeof(perf_event_header),
                                    sizeof(int64_t));
              if (rbTime < t0) {
                t0 = rbTime;
                sel = idx;
              ...
              rbPos += hdr.size;
              if (rbPos == end[idx]) {
                memmove(active + i, active + i + 1,
                        sizeof(*active) * (activeNum - i - 1));
                activeNum--;
                i--;
              } else {
                pos[idx] = rbPos;
          ...
          if (sel >= 0) {
            ...
            if (rid == EventContextSwitch) {
              ...
              //   u32 size
              //   u8  data[size]
              // Data (not ABI stable, but has not changed since it was added,
              // in 2009):
              //   u8  hdr[8]
              //   u8  prev_comm[16]
              //   u32 prev_pid
              ...
              // Note: The timestamp value t0 might be off by a number of
              // microseconds from the true hardware vblank event. The ktime
              // value should be used instead, but it is measured in
              // CLOCK_MONOTONIC time. Tracy only supports the timestamp counter
