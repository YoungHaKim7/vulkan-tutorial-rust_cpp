...
extern "C" {
typedef unsigned long(__stdcall *___tracy_t_RtlWalkFrameChain)(void **,
                                                               unsigned long,
                                                               unsigned long);
...
static _Unwind_Reason_Code tracy_unwind_callback(struct _Unwind_Context *ctx,
                                                 void *arg) {
...
static tracy_force_inline void *Callstack(int depth) {
  ...
  auto trace =
      (uintptr_t *)tracy_malloc((1 + (size_t)depth) * sizeof(uintptr_t));
