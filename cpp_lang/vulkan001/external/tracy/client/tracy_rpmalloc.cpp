...
static FORCEINLINE int32_t atomic_add32(atomic32_t *val, int32_t add) {
  return std::atomic_fetch_add_explicit(val, add, std::memory_order_relaxed) +
         add;
}
static FORCEINLINE int atomic_cas32_acquire(atomic32_t *dst, int32_t val,
                                            int32_t ref) {
  return std::atomic_compare_exchange_weak_explicit(
      dst, &ref, val, std::memory_order_acquire, std::memory_order_relaxed);
}
...
static FORCEINLINE int atomic_cas_ptr(atomicptr_t *dst, void *val, void *ref) {
  return std::atomic_compare_exchange_weak_explicit(
      dst, &ref, val, std::memory_order_relaxed, std::memory_order_relaxed);
}
...
static_assert((SMALL_GRANULARITY & (SMALL_GRANULARITY - 1)) == 0,
              "Small granularity must be power of two");
static_assert((SPAN_HEADER_SIZE & (SPAN_HEADER_SIZE - 1)) == 0,
...
TRACY_API void rpfree(void *ptr) { _rpmalloc_deallocate(ptr); }
