...
TRACY_API int64_t GetFrequencyQpc();
    ...
    (defined __i386 || defined _M_IX86 || defined __x86_64__ ||                \
     defined _M_X64)
TRACY_API bool
HardwareSupportsInvariantTSC(); // check, if we need fallback scenario
#else
...
typedef char *(*SourceContentsCallback)(void *data, const char *filename,
                                        size_t &size);
...
#error "TRACY_HW_TIMER detection logic needs fixing"
  ...
  static tracy_force_inline void SendFrameMark(const char *name,
    ...
    assert(type == QueueType::FrameMarkMsgStart ||
           type == QueueType::FrameMarkMsgEnd);
#ifdef TRACY_ON_DEMAND
    if (!GetProfiler().IsConnected())
      return;
#endif
  ...
  static tracy_force_inline void SendFrameImage(const void *image, uint16_t w,
                                                uint16_t h, uint8_t offset,
                                                bool flip) {
  ...
  static tracy_force_inline void ConfigurePlot(const char *name,
                                               PlotFormatType type, bool step,
  ...
  static tracy_force_inline void Message(const char *txt, size_t size,
    ...
    TracyQueuePrepare(callstack == 0 ? QueueType::Message
                                     : QueueType::MessageCallstack);
  ...
  static tracy_force_inline void Message(const char *txt, int callstack) {
    ...
    TracyQueuePrepare(callstack == 0 ? QueueType::MessageLiteral
                                     : QueueType::MessageLiteralCallstack);
  ...
  static tracy_force_inline void MessageColor(const char *txt, size_t size,
    ...
    TracyQueuePrepare(callstack == 0 ? QueueType::MessageColor
                                     : QueueType::MessageColorCallstack);
  ...
  static tracy_force_inline void MessageColor(const char *txt, uint32_t color,
    ...
    TracyQueuePrepare(callstack == 0 ? QueueType::MessageLiteralColor
  ...
  static tracy_force_inline void MemAllocCallstack(const void *ptr, size_t size,
                                                   int depth, bool secure) {
    if (secure && !ProfilerAvailable())
      return;
  ...
  static tracy_force_inline void MemAllocNamed(const void *ptr, size_t size,
  ...
  static tracy_force_inline void MemFreeNamed(const void *ptr, bool secure,
                                              const char *name) {
    if (secure && !ProfilerAvailable())
      return;
#ifdef TRACY_ON_DEMAND
  ...
  static tracy_force_inline void MemAllocCallstackNamed(const void *ptr,
  ...
  static tracy_force_inline void MemFreeCallstackNamed(const void *ptr,
                                                       int depth, bool secure,
                                                       const char *name) {
    if (secure && !ProfilerAvailable())
      return;
  ...
  static tracy_force_inline void ParameterRegister(ParameterCallback cb,
                                                   void *data) {
    auto &profiler = GetProfiler();
  ...
  static tracy_force_inline void ParameterSetup(uint32_t idx, const char *name,
                                                bool isBool, int32_t val) {
  ...
  SourceCallbackRegister(SourceContentsCallback cb, void *data) {
    auto &profiler = GetProfiler();
  ...
  static tracy_force_inline uint64_t AllocSourceLocation(uint32_t line,
                                                         const char *source,
                                                         const char *function) {
  ...
  static tracy_force_inline uint64_t AllocSourceLocation(uint32_t line,
                                                         ...
                                                         const char *name,
                                                         size_t nameSz) {
    return AllocSourceLocation(line, source, strlen(source), function,
                               strlen(function), name, nameSz);
  }
  ...
  static tracy_force_inline uint64_t AllocSourceLocation(uint32_t line,
                                                         ...
                                                         const char *function,
                                                         size_t functionSz) {
    return AllocSourceLocation(line, source, sourceSz, function, functionSz,
                               nullptr, 0);
  }

  static tracy_force_inline uint64_t AllocSourceLocation(
      uint32_t line, const char *source, size_t sourceSz, const char *function,
...
private:
  ...
  DequeueStatus DequeueContextSwitches(tracy::moodycamel::ConsumerToken &token,
                                       int64_t &timeStop);
  ...
  void SendLongString(uint64_t ptr, const char *str, size_t len,
                      QueueType type);
  ...
  static tracy_force_inline void SendMemAlloc(QueueType type,
                                              const uint32_t thread,
                                              const void *ptr, size_t size) {
    assert(type == QueueType::MemAlloc ||
           type == QueueType::MemAllocCallstack ||
           type == QueueType::MemAllocNamed ||
           type == QueueType::MemAllocCallstackNamed);
  ...
  static tracy_force_inline void
  SendMemFree(QueueType type, const uint32_t thread, const void *ptr) {
    assert(type == QueueType::MemFree || type == QueueType::MemFreeCallstack ||
           type == QueueType::MemFreeNamed ||
