...
meshopt_analyzeVertexCache(const unsigned int *indices, size_t index_count,
  ...
  for (size_t i = 0; i < index_count; i += 3) {
    ...
    // flush cache if triangle doesn't fit into warp or into the primitive
    // buffer
  ...
  result.acmr = index_count == 0 ? 0
                                 : float(result.vertices_transformed) /
                                       float(index_count / 3);
  result.atvr = unique_vertex_count == 0 ? 0
                                         : float(result.vertices_transformed) /
                                               float(unique_vertex_count);
