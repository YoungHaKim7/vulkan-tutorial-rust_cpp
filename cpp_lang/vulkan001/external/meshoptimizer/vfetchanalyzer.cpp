...
meshopt_analyzeVertexFetch(const unsigned int *indices, size_t index_count,
  ...
  // simple direct mapped cache; on typical mesh data this is close to 4-way
  // cache, and this model is a gross approximation anyway
  ...
  result.overfetch = unique_vertex_count == 0
                         ? 0
                         : float(result.bytes_fetched) /
                               float(unique_vertex_count * vertex_size);
