...
static float computeDepthGradients(float &dzdx, float &dzdy, float x1, float y1,
...
static void rasterize(OverdrawBuffer *buffer, float v1x, float v1y, float v1z,
                      float v2x, float v2y, float v2z, float v3x, float v3y,
                      float v3z) {
  // compute depth gradients
  float DZx, DZy;
  float det = computeDepthGradients(DZx, DZy, v1x, v1y, v1z, v2x, v2y, v2z, v3x,
                                    v3y, v3z);
  int sign = det > 0;
  ...
  if (sign) {
    ...
    // flip depth since we rasterize backfacing triangles to second buffer with
    // reverse Z; only v1z is used below
    v1z = kViewport - v1z;
    DZx = -DZx;
    DZy = -DZy;
  }
  ...
  // as for max, due to top-left filling convention we will never rasterize
  // right/bottom edges so max >= 0.5 should round down
  ...
  // note that we offset minx/miny by half pixel since we want to rasterize
  // pixels with covered centers
  int FX = (minx << 4) + 8;
  int FY = (miny << 4) + 8;
  ...
  for (int y = miny; y < maxy; y++) {
    ...
    for (int x = minx; x < maxx; x++) {
      ...
      // signed left shift is UB for negative numbers so use unsigned-signed
      // casts
...
meshopt_analyzeOverdraw(const unsigned int *indices, size_t index_count,
  ...
  float extent =
      max(maxv[0] - minv[0], max(maxv[1] - minv[1], maxv[2] - minv[2]));
  ...
  for (int axis = 0; axis < 3; ++axis) {
    ...
    for (size_t i = 0; i < index_count; i += 3) {
      ...
      switch (axis) {
      case 0:
        rasterize(buffer, vn0[2], vn0[1], vn0[0], vn1[2], vn1[1], vn1[0],
                  vn2[2], vn2[1], vn2[0]);
        break;
      case 1:
        rasterize(buffer, vn0[0], vn0[2], vn0[1], vn1[0], vn1[2], vn1[1],
                  vn2[0], vn2[2], vn2[1]);
        break;
      case 2:
        rasterize(buffer, vn0[1], vn0[0], vn0[2], vn1[1], vn1[0], vn1[2],
                  vn2[1], vn2[0], vn2[2]);
        break;
      }
    }
  ...
  result.overdraw = result.pixels_covered ? float(result.pixels_shaded) /
                                                float(result.pixels_covered)
                                          : 0.f;
