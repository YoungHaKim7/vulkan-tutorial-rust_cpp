...
static void buildPositionRemap(unsigned int *remap,
  ...
  for (size_t i = 0; i < vertex_count; ++i) {
    ...
    unsigned int *entry =
        hashLookup(vertex_table, vertex_table_size, vertex_hasher, index, ~0u);
  ...
  for (size_t i = 0; i < vertex_count; ++i) {
    if (remap[i] != ~0u) {
      ...
      memcpy(static_cast<unsigned char *>(destination) + remap[i] * vertex_size,
             static_cast<const unsigned char *>(vertices) + i * vertex_size,
             vertex_size);
    }
  }
}
...
void meshopt_generateAdjacencyIndexBuffer(unsigned int *destination,
  ...
  // build position remap: for each vertex, which other (canonical) vertex does
  // it map to?
  unsigned int *remap = allocator.allocate<unsigned int>(vertex_count);
  buildPositionRemap(remap, vertex_positions, vertex_count,
                     vertex_positions_stride, allocator);

  // build edge set; this stores all triangle edges but we can look these up by
  // any other wedge
  ...
  for (size_t i = 0; i < index_count; i += 3) {
    for (int e = 0; e < 3; ++e) {
      ...
      unsigned long long *entry =
          hashLookup(edge_table, edge_table_size, edge_hasher, edge, ~0ull);
  ...
  for (size_t i = 0; i < index_count; i += 3) {
    ...
    for (int e = 0; e < 3; ++e) {
      ...
      patch[e * 2 + 0] = i0;
      patch[e * 2 + 1] =
          (*oppe == ~0ull) ? i0 : edge_vertex_table[oppe - edge_table];
    }
