...
// Tuned to minimize the ACMR of a GPU that has a cache profile similar to
// NVidia and AMD
static const VertexScoreTable kVertexScoreTable = {
    {0.f, 0.779f, 0.791f, 0.789f, 0.981f, 0.843f, 0.726f, 0.847f, 0.882f,
     0.867f, 0.799f, 0.642f, 0.613f, 0.600f, 0.568f, 0.372f, 0.234f},
...
static const VertexScoreTable kVertexScoreTableStrip = {
    {0.f, 1.000f, 1.000f, 1.000f, 0.453f, 0.561f, 0.490f, 0.459f, 0.179f,
     0.526f, 0.000f, 0.227f, 0.184f, 0.490f, 0.112f, 0.050f, 0.131f},
...
static void buildTriangleAdjacency(TriangleAdjacency &adjacency,
...
static unsigned int getNextVertexDeadEnd(const unsigned int *dead_end,
                                         unsigned int &dead_end_top,
                                         unsigned int &input_cursor,
                                         const unsigned int *live_triangles,
                                         size_t vertex_count) {
...
static unsigned int
getNextVertexNeighbour(const unsigned int *next_candidates_begin,
                       const unsigned int *next_candidates_end,
                       const unsigned int *live_triangles,
                       const unsigned int *cache_timestamps,
  ...
  for (const unsigned int *next_candidate = next_candidates_begin;
    ...
    if (live_triangles[vertex] > 0) {
      ...
      if (2 * live_triangles[vertex] + timestamp - cache_timestamps[vertex] <=
          cache_size) {
...
static float vertexScore(const VertexScoreTable *table, int cache_position,
...
static unsigned int getNextTriangleDeadEnd(unsigned int &input_cursor,
                                           const unsigned char *emitted_flags,
                                           size_t face_count) {
  // input order
...
void meshopt_optimizeVertexCacheTable(unsigned int *destination,
                                      const unsigned int *indices,
                                      size_t index_count, size_t vertex_count,
                                      const meshopt::VertexScoreTable *table) {
  ...
  while (current_triangle != ~0u) {
    ...
    // update cache positions, vertex scores and triangle scores, and find next
    // best triangle
    for (size_t i = 0; i < cache_write; ++i) {
      ...
      const unsigned int *neighbours_begin =
          &adjacency.data[0] + adjacency.offsets[index];
      const unsigned int *neighbours_end =
          neighbours_begin + adjacency.counts[index];

      for (const unsigned int *it = neighbours_begin; it != neighbours_end;
           ++it) {
        unsigned int tri = *it;
    ...
    if (current_triangle == ~0u) {
      current_triangle =
          getNextTriangleDeadEnd(input_cursor, &emitted_flags[0], face_count);
    }
  }
...
void meshopt_optimizeVertexCache(unsigned int *destination,
                                 const unsigned int *indices,
                                 size_t index_count, size_t vertex_count) {
  meshopt_optimizeVertexCacheTable(destination, indices, index_count,
                                   vertex_count, &meshopt::kVertexScoreTable);
}

void meshopt_optimizeVertexCacheStrip(unsigned int *destination,
                                      const unsigned int *indices,
                                      size_t index_count, size_t vertex_count) {
  meshopt_optimizeVertexCacheTable(destination, indices, index_count,
                                   vertex_count,
                                   &meshopt::kVertexScoreTableStrip);
}

void meshopt_optimizeVertexCacheFifo(unsigned int *destination,
                                     const unsigned int *indices,
                                     size_t index_count, size_t vertex_count,
  ...
  while (current_vertex != ~0u) {
    ...
    const unsigned int *neighbours_end =
        neighbours_begin + adjacency.counts[current_vertex];
    ...
    // get next vertex
    current_vertex = getNextVertexNeighbour(
        next_candidates_begin, next_candidates_end, &live_triangles[0],
        &cache_timestamps[0], timestamp, cache_size);

    if (current_vertex == ~0u) {
      current_vertex =
          getNextVertexDeadEnd(&dead_end[0], dead_end_top, input_cursor,
                               &live_triangles[0], vertex_count);
    }
  }
