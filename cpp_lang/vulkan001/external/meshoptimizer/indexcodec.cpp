...
static const unsigned char kCodeAuxEncodingTable[16] = {
    0x00, 0x76, 0x87, 0x56, 0x67, 0x78, 0xa9, 0x86, 0x65,
    0x89, 0x68, 0x98, 0x01, 0x69, 0,    0, // last two entries aren't used for
                                           // encoding
};
...
static int getEdgeFifo(EdgeFifo fifo, unsigned int a, unsigned int b,
                       unsigned int c, size_t offset) {
  for (int i = 0; i < 16; ++i) {
...
static void pushEdgeFifo(EdgeFifo fifo, unsigned int a, unsigned int b,
                         size_t &offset) {
  fifo[offset][0] = a;
  fifo[offset][1] = b;
  offset = (offset + 1) & 15;
...
static unsigned int decodeVByte(const unsigned char *&data) {
  ...
  // note that this loop always terminates, which is important for malformed
  // data
...
static void encodeIndex(unsigned char *&data, unsigned int index,
                        unsigned int last) {
...
static void writeTriangle(void *destination, size_t offset, size_t index_size,
  ...
  if (index_size == 2) {
    static_cast<unsigned short *>(destination)[offset + 0] =
        (unsigned short)(a);
    static_cast<unsigned short *>(destination)[offset + 1] =
        (unsigned short)(b);
    static_cast<unsigned short *>(destination)[offset + 2] =
        (unsigned short)(c);
  } else {
...
size_t meshopt_encodeIndexBuffer(unsigned char *buffer, size_t buffer_size,
  ...
  // the minimum valid encoding is header, 1 byte per triangle and a 16-byte
  // codeaux table
  ...
  // use static encoding table; it's possible to pack the result and then build
  // an optimal table and repack for now we keep it simple and use the table
  // that has been generated based on symbol frequency on a training mesh set
  const unsigned char *codeaux_table = kCodeAuxEncodingTable;

  for (size_t i = 0; i < index_count; i += 3) {
    // make sure we have enough space to write a triangle
    // each triangle writes at most 16 bytes: 1b for codeaux and 5b for each
    // free index after this we can be sure we can write without extra bounds
    // checks
    if (data > data_safe_end)
      return 0;

    int fer = getEdgeFifo(edgefifo, indices[i + 0], indices[i + 1],
                          indices[i + 2], edgefifooffset);

    if (fer >= 0 && (fer >> 2) < 15) {
      const unsigned int *order = kTriangleIndexOrder[fer & 3];

      unsigned int a = indices[i + order[0]], b = indices[i + order[1]],
                   c = indices[i + order[2]];
      ...
      *code++ = (unsigned char)((fe << 4) | fec);
      ...
      // note that we need to update the last index since free indices are
      // delta-encoded
      if (fec == 15)
        encodeIndex(data, c, last), last = c;

      // we only need to push third vertex since first two are likely already in
      // the vertex fifo
      if (fec == 0 || fec >= fecmax)
        pushVertexFifo(vertexfifo, c, vertexfifooffset);

      // we only need to push two new edges to edge fifo since the third one is
      // already there
      pushEdgeFifo(edgefifo, c, b, edgefifooffset);
      pushEdgeFifo(edgefifo, a, c, edgefifooffset);
    } else {
      int rotation =
          rotateTriangle(indices[i + 0], indices[i + 1], indices[i + 2], next);
      ...
      if (a == 0 && b == 1 && c == 2 && next > 0 && version >= 1) {
        ...
        // reset vertex fifo to make sure we don't accidentally reference
        // vertices from that in the future this makes sure next continues to
        // get incremented instead of being stuck
        memset(vertexfifo, -1, sizeof(vertexfifo));
      }
      ...
      // after rotation, a is almost always equal to next, so we don't waste
      // bits on FIFO encoding for a
      int fea = (a == next) ? (next++, 0) : 15;
      int feb = (fb >= 0 && fb < 14) ? (fb + 1)
                : (b == next)        ? (next++, 0)
                                     : 15;
      int fec = (fc >= 0 && fc < 14) ? (fc + 1)
                : (c == next)        ? (next++, 0)
                                     : 15;

      // we encode feb & fec in 4 bits using a table if possible, and as a full
      // byte otherwise
      unsigned char codeaux = (unsigned char)((feb << 4) | fec);
      int codeauxindex = getCodeAuxIndex(codeaux, codeaux_table);

      // <14 encodes an index into codeaux table, 14 encodes fea=0, 15 encodes
      // fea=15
      ...
      // all three edges aren't in the fifo; pushing all of them is important so
      // that we can match them for later triangles
  ...
  // add codeaux encoding table to the end of the stream; this is used for
  // decoding codeaux *and* as padding we need padding for decoding to be able
  // to assume that each triangle is encoded as <= 16 bytes of extra data this
  // is enough space for aux byte + 5 bytes per varint index which is the
  // absolute worst case for any input
  for (size_t i = 0; i < 16; ++i) {
    // decoder assumes that table entries never refer to separately encoded
    // indices
  ...
  // since we encode restarts as codeaux without a table reference, we need to
  // make sure 00 is encoded as a table reference
...
int meshopt_decodeIndexBuffer(void *destination, size_t index_count,
  ...
  // since we store 16-byte codeaux table at the end, triangle data has to begin
  // before data_safe_end
  ...
  for (size_t i = 0; i < index_count; i += 3) {
    ...
    // each triangle reads at most 16 bytes of data: 1b for codeaux and 5b for
    // each free index after this we can be sure we can read without extra
    // bounds checks
    if (data > data_safe_end)
      return -2;
    ...
    if (codetri < 0xf0) {
      ...
      // inside this if we try to stay branchless (by using cmov/etc.) since
      ...
      if (fec < fecmax) {
        ...
        // push vertex/edge fifo must match the encoding step *exactly*
        ...
        // delta-encoded
        last = c =
            (fec != 15) ? last + (fec - (fec ^ 3)) : decodeIndex(data, last);
      ...
      if (codetri < 0xfe) {
        ...
        // fifo reads are wrapped around 16 entry buffer
        // also note that we increment next for all three vertices before
        // decoding indices - this matches encoder behavior
        unsigned int a = next++;
        ...
        // slow path: read a full byte for codeaux instead of using a table
        // lookup
        ...
        unsigned int b =
            (feb == 0) ? next++ : vertexfifo[(vertexfifooffset - feb) & 15];
        unsigned int c =
            (fec == 0) ? next++ : vertexfifo[(vertexfifooffset - fec) & 15];
  ...
  // we should've read all data bytes and stopped at the boundary between data
  // and codeaux table
  if (data != data_safe_end)
    return -3;
...
size_t meshopt_encodeIndexSequence(unsigned char *buffer, size_t buffer_size,
                                   const unsigned int *indices,
                                   size_t index_count) {
  using namespace meshopt;
  ...
  for (size_t i = 0; i < index_count; ++i) {
    ...
    // each index writes at most 5 bytes of data; there's a 4 byte tail after
    // data_safe_end after this we can be sure we can write without extra bounds
    // checks
    if (data >= data_safe_end)
      return 0;
    ...
    // this is a heuristic that switches between baselines when the delta grows
    // too large we want the encoded delta to fit into one byte (7 bits), but 2
    // bits are used for sign and baseline index for now we immediately switch
    // the baseline when delta grows too large - this can be adjusted
    // arbitrarily
    ...
    // note: low bit encodes the index of the last baseline which will be used
    // for reconstruction
...
size_t meshopt_encodeIndexSequenceBound(size_t index_count,
                                        size_t vertex_count) {
  // compute number of bits required for each index
  unsigned int vertex_bits = 1;
  ...
  // worst-case encoding is 1 varint-7 encoded index delta for a K bit value and
  // an extra bit
...
int meshopt_decodeIndexSequence(void *destination, size_t index_count,
                                size_t index_size, const unsigned char *buffer,
                                size_t buffer_size) {
  using namespace meshopt;
  ...
  for (size_t i = 0; i < index_count; ++i) {
    ...
    // each index reads at most 5 bytes of data; there's a 4 byte tail after
    // data_safe_end after this we can be sure we can read without extra bounds
    // checks
    if (data >= data_safe_end)
      return -2;
