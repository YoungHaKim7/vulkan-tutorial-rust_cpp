...
// The SIMD implementation requires SSE2, which can be enabled unconditionally
// through compiler settings
#if defined(__SSE2__)
#define SIMD_SSE
#endif

// MSVC supports compiling SSE2 code regardless of compile options; we assume
// all 32-bit CPUs support SSE2
#if !defined(SIMD_SSE) && defined(_MSC_VER) && !defined(__clang__) &&          \
...
template <typename T> static void decodeFilterOct(T *data, size_t count) {
  ...
  for (size_t i = 0; i < count; ++i) {
    // convert x and y to floats and reconstruct z; this assumes zf encodes 1.f
...
static void decodeFilterQuat(short *data, size_t count) {
  ...
  for (size_t i = 0; i < count; ++i) {
    ...
    // reconstruct w as a square root; we clamp to 0.f to avoid NaN due to
    // precision errors
...
#if defined(SIMD_SSE) || defined(SIMD_NEON) || defined(SIMD_WASM)
template <typename T>
static void dispatchSimd(void (*process)(T *, size_t), T *data, size_t count,
                         size_t stride) {
  assert(stride <= 4);
...
static void decodeFilterOctSimd(signed char *data, size_t count) {
  ...
  for (size_t i = 0; i < count; i += 4) {
    ...
    // unpack z; note that z is unsigned so we technically don't need to sign
    // extend it
    ...
    __m128 z =
        _mm_sub_ps(_mm_cvtepi32_ps(zf),
                   _mm_add_ps(_mm_andnot_ps(sign, x), _mm_andnot_ps(sign, y)));
    ...
    // compute normal length & scale
    __m128 ll = _mm_add_ps(_mm_mul_ps(x, x),
                           _mm_add_ps(_mm_mul_ps(y, y), _mm_mul_ps(z, z)));
    ...
    res = _mm_or_si128(
        res, _mm_slli_epi32(_mm_and_si128(yr, _mm_set1_epi32(0xff)), 8));
    res = _mm_or_si128(
        res, _mm_slli_epi32(_mm_and_si128(zr, _mm_set1_epi32(0xff)), 16));
...
static void decodeFilterOctSimd(short *data, size_t count) {
  ...
  for (size_t i = 0; i < count; i += 4) {
    ...
    __m128i n4 =
        _mm_castps_si128(_mm_shuffle_ps(n4_0, n4_1, _MM_SHUFFLE(2, 0, 2, 0)));
    ...
    __m128i z4 =
        _mm_castps_si128(_mm_shuffle_ps(n4_0, n4_1, _MM_SHUFFLE(3, 1, 3, 1)));
    ...
    __m128i xzr = _mm_or_si128(_mm_and_si128(xr, _mm_set1_epi32(0xffff)),
                               _mm_slli_epi32(zr, 16));
    __m128i y0r = _mm_and_si128(yr, _mm_set1_epi32(0xffff));

    // pack x/y/z using 16-bit unpacks; note that this has 0 where we should
    // have .w
    ...
    // patch in .w
    res_0 =
        _mm_or_si128(res_0, _mm_and_si128(_mm_castps_si128(n4_0),
                                          _mm_set1_epi64x(0xffff000000000000)));
    res_1 =
        _mm_or_si128(res_1, _mm_and_si128(_mm_castps_si128(n4_1),
                                          _mm_set1_epi64x(0xffff000000000000)));
...
static void decodeFilterQuatSimd(short *data, size_t count) {
  ...
  for (size_t i = 0; i < count; i += 4) {
    ...
    // gather both x/y 16-bit pairs in each 32-bit lane
    __m128i q4_xy =
        _mm_castps_si128(_mm_shuffle_ps(q4_0, q4_1, _MM_SHUFFLE(2, 0, 2, 0)));
    __m128i q4_zc =
        _mm_castps_si128(_mm_shuffle_ps(q4_0, q4_1, _MM_SHUFFLE(3, 1, 3, 1)));
    ...
    // get a floating-point scaler using zc with bottom 2 bits set to 1 (which
    // represents 1.f)
    ...
    // precision errors
    __m128 ww =
        _mm_sub_ps(_mm_set1_ps(1.f),
                   _mm_add_ps(_mm_mul_ps(x, x),
                              _mm_add_ps(_mm_mul_ps(y, y), _mm_mul_ps(z, z))));
    ...
    __m128i xzr = _mm_or_si128(_mm_and_si128(xr, _mm_set1_epi32(0xffff)),
                               _mm_slli_epi32(zr, 16));
    __m128i wyr = _mm_or_si128(_mm_and_si128(wr, _mm_set1_epi32(0xffff)),
                               _mm_slli_epi32(yr, 16));
...
static void decodeFilterOctSimd(signed char *data, size_t count) {
  ...
  for (size_t i = 0; i < count; i += 4) {
    ...
    float32x4_t z =
        vsubq_f32(vcvtq_f32_s32(zf), vaddq_f32(vabsq_f32(x), vabsq_f32(y)));

    // fixup octahedral coordinates for z<0
    float32x4_t t = vminq_f32(z, vdupq_n_f32(0.f));

    x = vaddq_f32(x, vreinterpretq_f32_s32(
                         veorq_s32(vreinterpretq_s32_f32(t),
                                   vandq_s32(vreinterpretq_s32_f32(x), sign))));
    y = vaddq_f32(y, vreinterpretq_f32_s32(
                         veorq_s32(vreinterpretq_s32_f32(t),
                                   vandq_s32(vreinterpretq_s32_f32(y), sign))));

    // compute normal length & scale
    float32x4_t ll =
        vaddq_f32(vmulq_f32(x, x), vaddq_f32(vmulq_f32(y, y), vmulq_f32(z, z)));
    float32x4_t rl = vrsqrteq_f32(ll);
    float32x4_t s = vmulq_f32(vdupq_n_f32(127.f), rl);

    // fast rounded signed float->int: addition triggers renormalization after
    // which mantissa stores the integer value note: the result is offset by
    // 0x4B40_0000, but we only need the low 16 bits so we can omit the
    // subtraction
...
static void decodeFilterOctSimd(short *data, size_t count) {
  ...
  for (size_t i = 0; i < count; i += 4) {
    ...
    float32x4_t rl = vrsqrteq_f32(ll);
    rl = vmulq_f32(
        rl, vrsqrtsq_f32(vmulq_f32(rl, ll), rl)); // refine rsqrt estimate
    ...
    // mix x/z and y/0 to make 16-bit unpack easier
    int32x4_t xzr =
        vorrq_s32(vandq_s32(xr, vdupq_n_s32(0xffff)), vshlq_n_s32(zr, 16));
    int32x4_t y0r = vandq_s32(yr, vdupq_n_s32(0xffff));

    // pack x/y/z using 16-bit unpacks; note that this has 0 where we should
    // have .w
    int32x4_t res_0 = vreinterpretq_s32_s16(
        vzipq_s16(vreinterpretq_s16_s32(xzr), vreinterpretq_s16_s32(y0r))
            .val[0]);
    int32x4_t res_1 = vreinterpretq_s32_s16(
        vzipq_s16(vreinterpretq_s16_s32(xzr), vreinterpretq_s16_s32(y0r))
            .val[1]);

    // patch in .w
    res_0 = vbslq_s32(vreinterpretq_u32_u64(vdupq_n_u64(0xffff000000000000)),
                      n4_0, res_0);
    res_1 = vbslq_s32(vreinterpretq_u32_u64(vdupq_n_u64(0xffff000000000000)),
                      n4_1, res_1);
...
static void decodeFilterQuatSimd(short *data, size_t count) {
  ...
  for (size_t i = 0; i < count; i += 4) {
    ...
    int32x4_t xzr =
        vorrq_s32(vandq_s32(xr, vdupq_n_s32(0xffff)), vshlq_n_s32(zr, 16));
    int32x4_t wyr =
        vorrq_s32(vandq_s32(wr, vdupq_n_s32(0xffff)), vshlq_n_s32(yr, 16));

    // pack x/y/z/w using 16-bit unpacks; we pack wxyz by default (for qc=0)
    int32x4_t res_0 = vreinterpretq_s32_s16(
        vzipq_s16(vreinterpretq_s16_s32(wyr), vreinterpretq_s16_s32(xzr))
            .val[0]);
    int32x4_t res_1 = vreinterpretq_s32_s16(
        vzipq_s16(vreinterpretq_s16_s32(wyr), vreinterpretq_s16_s32(xzr))
            .val[1]);
    ...
    out[0] = rotateleft64(vgetq_lane_u64(vreinterpretq_u64_s32(res_0), 0),
                          vgetq_lane_s32(cf, 0) << 4);
    out[1] = rotateleft64(vgetq_lane_u64(vreinterpretq_u64_s32(res_0), 1),
                          vgetq_lane_s32(cf, 1) << 4);
    out[2] = rotateleft64(vgetq_lane_u64(vreinterpretq_u64_s32(res_1), 0),
                          vgetq_lane_s32(cf, 2) << 4);
    out[3] = rotateleft64(vgetq_lane_u64(vreinterpretq_u64_s32(res_1), 1),
                          vgetq_lane_s32(cf, 3) << 4);
  }
}
...
static void decodeFilterOctSimd(signed char *data, size_t count) {
  ...
  for (size_t i = 0; i < count; i += 4) {
    ...
    v128_t z =
        wasm_f32x4_sub(wasm_f32x4_convert_i32x4(zf),
                       wasm_f32x4_add(wasm_f32x4_abs(x), wasm_f32x4_abs(y)));
    ...
    // compute normal length & scale
    v128_t ll = wasm_f32x4_add(
        wasm_f32x4_mul(x, x),
        wasm_f32x4_add(wasm_f32x4_mul(y, y), wasm_f32x4_mul(z, z)));
    v128_t s = wasm_f32x4_div(wasm_f32x4_splat(127.f), wasm_f32x4_sqrt(ll));

    // fast rounded signed float->int: addition triggers renormalization after
    // which mantissa stores the integer value note: the result is offset by
    // 0x4B40_0000, but we only need the low 8 bits so we can omit the
    // subtraction
    ...
    res = wasm_v128_or(
        res, wasm_i32x4_shl(wasm_v128_and(yr, wasm_i32x4_splat(0xff)), 8));
    res = wasm_v128_or(
        res, wasm_i32x4_shl(wasm_v128_and(zr, wasm_i32x4_splat(0xff)), 16));
...
static void decodeFilterOctSimd(short *data, size_t count) {
  ...
  for (size_t i = 0; i < count; i += 4) {
    ...
    // mix x/z and y/0 to make 16-bit unpack easier
    v128_t xzr = wasm_v128_or(wasm_v128_and(xr, wasm_i32x4_splat(0xffff)),
                              wasm_i32x4_shl(zr, 16));
    ...
    // patch in .w
    res_0 = wasm_v128_or(
        res_0, wasm_v128_and(n4_0, wasm_i64x2_splat(0xffff000000000000)));
    res_1 = wasm_v128_or(
        res_1, wasm_v128_and(n4_1, wasm_i64x2_splat(0xffff000000000000)));
...
static void decodeFilterQuatSimd(short *data, size_t count) {
  ...
  for (size_t i = 0; i < count; i += 4) {
    ...
    v128_t ss =
        wasm_f32x4_div(wasm_f32x4_splat(scale), wasm_f32x4_convert_i32x4(sf));
    ...
    // reconstruct w as a square root; we clamp to 0.f to avoid NaN due to
    // precision errors note: i32x4_max with 0 is equivalent to f32x4_max
    v128_t ww =
        wasm_f32x4_sub(wasm_f32x4_splat(1.f),
                       wasm_f32x4_add(wasm_f32x4_mul(x, x),
                                      wasm_f32x4_add(wasm_f32x4_mul(y, y),
                                                     wasm_f32x4_mul(z, z))));
    ...
    v128_t xzr = wasm_v128_or(wasm_v128_and(xr, wasm_i32x4_splat(0xffff)),
                              wasm_i32x4_shl(zr, 16));
    v128_t wyr = wasm_v128_or(wasm_v128_and(wr, wasm_i32x4_splat(0xffff)),
                              wasm_i32x4_shl(yr, 16));
    ...
    out[0] = rotateleft64(wasm_i64x2_extract_lane(res_0, 0),
                          wasm_i32x4_extract_lane(cm, 0));
    out[1] = rotateleft64(wasm_i64x2_extract_lane(res_0, 1),
                          wasm_i32x4_extract_lane(cm, 1));
    out[2] = rotateleft64(wasm_i64x2_extract_lane(res_1, 0),
                          wasm_i32x4_extract_lane(cm, 2));
    out[3] = rotateleft64(wasm_i64x2_extract_lane(res_1, 1),
                          wasm_i32x4_extract_lane(cm, 3));
  }
}
...
#if defined(SIMD_SSE) || defined(SIMD_NEON) || defined(SIMD_WASM)
  if (stride == 4)
    dispatchSimd(decodeFilterOctSimd, static_cast<signed char *>(buffer), count,
                 4);
  else
...
#if defined(SIMD_SSE) || defined(SIMD_NEON) || defined(SIMD_WASM)
  dispatchSimd(decodeFilterExpSimd, static_cast<unsigned int *>(buffer),
               count * (stride / 4), 1);
#else
...
void meshopt_encodeFilterOct(void *destination, size_t count, size_t stride,
...
void meshopt_encodeFilterQuat(void *destination_, size_t count, size_t stride,
                              int bits, const float *data) {
  assert(stride == 8);
  ...
  for (size_t i = 0; i < count; ++i) {
    ...
    // note: we always encode a cyclical swizzle to be able to recover the order
    // via rotation
...
void meshopt_encodeFilterExp(void *destination_, size_t count, size_t stride,
                             int bits, const float *data) {
  assert(stride > 0 && stride % 4 == 0);
  ...
  for (size_t i = 0; i < count; ++i) {
    ...
    // use maximum exponent to encode values; this guarantess that mantissa is
    // [-1, 1]
    int exp = -100;
    ...
    // note that we additionally scale the mantissa to make it a K-bit signed
    // integer (K-1 bits for magnitude)
    exp -= (bits - 1);
