...
// Francine Evans, Steven Skiena and Amitabh Varshney. Optimizing Triangle
// Strips for Fast Rendering. 1996
namespace meshopt {

static unsigned int findStripFirst(const unsigned int buffer[][3],
  ...
  for (size_t i = 0; i < buffer_size; ++i) {
    unsigned int va = valence[buffer[i][0]], vb = valence[buffer[i][1]],
...
static int findStripNext(const unsigned int buffer[][3],
                         unsigned int buffer_size, unsigned int e0,
                         unsigned int e1) {
  for (size_t i = 0; i < buffer_size; ++i) {
...
size_t meshopt_stripify(unsigned int *destination, const unsigned int *indices,
  ...
  // compute vertex valence; this is used to prioritize starting triangle for
  // strips
  ...
  while (buffer_size > 0 || index_offset < index_count) {
    ...
    if (next >= 0) {
      ...
      memmove(buffer[i], buffer[i + 1],
              (buffer_size - i - 1) * sizeof(buffer[0]));
      buffer_size--;
      ...
      // in some cases we need to perform a swap to pick a different outgoing
      // triangle edge for [a b c], the default strip edge is [b c], but we
      // might want to use [a c]
      int cont = findStripNext(buffer, buffer_size, parity ? strip[1] : v,
                               parity ? v : strip[1]);
      int swap = cont < 0
                     ? findStripNext(buffer, buffer_size, parity ? v : strip[0],
                                     parity ? strip[0] : v)
                     : -1;
      ...
      // we need to pre-rotate the triangle so that we will find a match in the
      ...
      // in some cases we can have several matching edges; since we can pick any
      // edge, we pick the one with the smallest triangle index in the buffer.
      // this reduces the effect of stripification on ACMR and additionally -
...
size_t meshopt_stripifyBound(size_t index_count) {
  ...
  // worst case without restarts is 2 degenerate indices and 3 indices per
  // triangle worst case with restarts is 1 restart index and 3 indices per
  // triangle
...
size_t meshopt_unstripify(unsigned int *destination,
  ...
  for (size_t i = 0; i < index_count; ++i) {
    if (restart_index && indices[i] == restart_index) {
      ...
      // although we use restart indices, strip swaps still produce degenerate
      // triangles, so skip them
      if (a != b && a != c && b != c) {
