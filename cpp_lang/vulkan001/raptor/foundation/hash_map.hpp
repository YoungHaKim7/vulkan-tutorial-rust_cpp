...
template <> inline u64 hash_calculate(const cstring &value, sizet seed) {
  return wyhash(value, strlen(value), seed, _wyp);
}
...
// Implementation details: the low bits of the pointer have little or no entropy
// because of alignment. We shift the pointer to try to use higher entropy bits.
// A good number seems to be 12 bits, because that aligns with page size.
...
// FlatHashMap ////////////////////////////////////////////////////////
template <typename K, typename V> void FlatHashMap<K, V>::reset_ctrl() {
...
template <typename K, typename V> void FlatHashMap<K, V>::reset_growth_left() {
...
template <typename K, typename V> u32 FlatHashMap<K, V>::remove(const K &key) {
...
template <typename K, typename V> void FlatHashMap<K, V>::initialize_slots() {
...
FlatHashMap<K, V>::get_structure(const K &key) {
  FlatHashMapIterator iterator = find(key);
  if (iterator.index != k_iterator_end)
...
template <typename K, typename V>
typename FlatHashMap<K, V>::KeyValue &
FlatHashMap<K, V>::get_structure(const FlatHashMapIterator &iterator) {
  return slots_[iterator.index];
}
...
template <typename K, typename V>
inline void
FlatHashMap<K, V>::iterator_skip_empty_or_deleted(FlatHashMapIterator &it) {
