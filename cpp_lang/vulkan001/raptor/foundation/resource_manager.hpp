...
struct ResourceLoader {
  ...
  virtual Resource *
  create_from_file(cstring name, cstring filename,
...
struct ResourceManager {
  ...
  template <typename T> T *load(cstring name);

  template <typename T> T *get(cstring name);

  template <typename T> T *get(u64 hashed_name);

  template <typename T> T *reload(cstring name);
...
template <typename T> inline T *ResourceManager::load(cstring name) {
...
template <typename T> inline T *ResourceManager::get(cstring name) {
  ResourceLoader *loader = loaders.get(T::k_type_hash);
  if (loader) {
...
template <typename T> inline T *ResourceManager::get(u64 hashed_name) {
  ResourceLoader *loader = loaders.get(T::k_type_hash);
  if (loader) {
...
template <typename T> inline T *ResourceManager::reload(cstring name) {
  ResourceLoader *loader = loaders.get(T::k_type_hash);
  if (loader) {
    T *resource = (T *)loader->get(name);
    if (resource) {
