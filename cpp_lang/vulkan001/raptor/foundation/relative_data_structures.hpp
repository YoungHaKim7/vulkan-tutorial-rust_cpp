...
//
//
template <typename T> struct RelativePointer {
...
//
//
template <typename T> struct RelativeArray {
...
template <typename T> inline bool RelativePointer<T>::is_null() const {
  return offset == 0;
}

template <typename T> inline bool RelativePointer<T>::is_not_null() const {
  return offset != 0;
}
...
template <typename T> inline void RelativePointer<T>::set(char *raw_pointer) {
  offset = raw_pointer ? (i32)(raw_pointer - (char *)this) : 0;
}
template <typename T> inline void RelativePointer<T>::set_null() { offset = 0; }
#endif // RAPTOR_BLOB_WRITE
...
template <typename T> inline T &RelativeArray<T>::operator[](u32 index) {
  RASSERT(index < size);
  return data.get()[index];
}

template <typename T> inline const T *RelativeArray<T>::get() const {
  return data.get();
}

template <typename T> inline T *RelativeArray<T>::get() { return data.get(); }
...
template <typename T> inline void RelativeArray<T>::set_empty() {
  size = 0;
  data.set_null();
}
#endif // RAPTOR_BLOB_WRITE
