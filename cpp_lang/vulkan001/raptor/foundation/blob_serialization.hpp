...
struct BlobSerializer {
  ...
  // Allocator is used to allocate memory if needed (for example when reading an
  // array).
  template <typename T>
  ...
  template <typename T> void serialize(RelativePointer<T> *data);

  template <typename T> void serialize(RelativeArray<T> *data);

  template <typename T> void serialize(Array<T> *data);
  ...
  char *allocate_static(sizet size); // Just allocate size bytes and return.
                                     // Used to fill in structures.

  template <typename T> T *allocate_static();
...
T *BlobSerializer::write_and_prepare(Allocator *allocator_,
  ...
  // Allocate root data. BlobHeader is already allocated in the write_common
  // method.
...
template <typename T> inline T *BlobSerializer::allocate_static() {
...
template <typename T> inline void BlobSerializer::serialize(Array<T> *data) {
  ...
  if (is_reading) {
    ...
    // Data --> Blob
    serialize(&data->size);
    // Add serialization pads so that we serialize all bytes of the struct
    // Array.
...
template <typename T> inline void BlobSerializer::serialize(T *data) {
